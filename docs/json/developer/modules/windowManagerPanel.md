...
-   **Open Windows:** Provides an "Open Window" button that publishes a `window:loadUrl` event; each open `windowPanel` answers it by calling `window.open()` itself, so with no Window panel open nothing opens.
