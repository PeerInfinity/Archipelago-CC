...
- **Load from File:** Allows the user to upload a previously saved combined JSON file. This will apply the configurations and data from the file to the current session. `rulesConfig` is applied live by publishing `files:jsonLoaded`. Non-reloading module data (like `stateManagerRuntime` and `stateManagerSnapshot`) is applied live. `layoutConfig` is deferred until the next page reload.
