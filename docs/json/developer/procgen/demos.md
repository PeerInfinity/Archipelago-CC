...
| **the REFERENCE** | The tables the CODE knows — every URL parameter of both lab pages, the generation catalogue and the refusal vocabulary — at <https://peerinfinity.github.io/Archipelago-CC/modules/procgenDocs/reference.html>, GENERATED by [`scripts/procgen/generate-procgen-reference.mjs`](../../../../scripts/procgen/generate-procgen-reference.mjs) and gated by `--check` (regenerate = no diff). ⛔ A demo's URL is authored in `demos.js`; what a parameter in it MEANS is generated there. |
