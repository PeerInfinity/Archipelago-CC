...
**Reset-teleport semantics.** A wrong region change while a bot drives (an open non-target portal, the player grabbing the controls) hard-pauses the queue — the same wrong-exit guard Manual uses. A **loop-reset teleport is different and must never be treated as a wrong exit**: `_handleBotWake_regionChanged` takes a `fromReset` flag (mirroring the manual wake's `fromReset` exemption), and on a reset teleport it *releases* the bot park (`_stopBotExecutedAction`) and *resumes the frame loop* rather than pausing. Resuming is load-bearing: a bot park leaves the RAF loop dormant (`_animationFrameId` null, `isProcessing` still true), and a fork-propagated reset runs only `_resetActionsProgress()` (cursor → 0) — so nothing else would re-schedule a frame, and the queue would sit dormant with a stale park. Resuming lets the Bot branch re-park and re-dispatch from the top: the **generic queue-restart retry** (the path a solver with no bridge memory relies on — see [jta.md](./jta.md#playback--bot-execution) for the contrast with jta's bridge-held pending walk). A multi-region walk that teleported away additionally needs the queue routed back to the cursor's region (its own leading moves, or the caller) before the re-dispatch lands — that routing is the queue's concern, not the wake's. *(Written for omsi arc D as much as jta: any solver substrate whose native economy resets by teleporting to a start region hits this path.)*
...
- **Enforcement is staged per substrate**: the gate (and loops-side charging) applies only where the substrate declares `loopSupport.record && loopSupport.playback` — maze and the text adventure since M3b, jta since M4, and runner and bounce since M5. Only omsi and flash remain unadopted; omsi arc D declares the capabilities to opt in, and flash is manual-only by design.
