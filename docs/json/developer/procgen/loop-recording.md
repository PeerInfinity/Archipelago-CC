...
⛓ A **lab walk** is the same envelope: the maze lab page's MANUAL arm (`?source=manual`, slice S2b) folds a hand-driven walk into a `SavedQueue` with `regionName: null` plus an additive `lab` block naming the level it was driven on, so the day a lab level is a region the walk is persistable under that region's tag with nothing rewritten. Loops never reads `lab` and the lab never writes the store (`saveQueue` needs a rules hash and a region name, and a lab level has neither) — see [Maze Substrate](./maze.md) § *The MANUAL arm*.
...
| Cost of one attempt | sub-second under Instant | **~12 s** — every fork loop end is a full host round trip. Instant (2026-07-25) collapses the *ticking inside* a run, not the round trips between them, so this rate is what an omsi bot is still sized against |
...
None of that contradicts the ruling — a present player still presses Start — but it does mean **any automated multi-run walk must set `autoRestartQueue` ON**, which is the flag's own contract ("auto-restart IS the retry", already the case in the jta bot legs). The omsi bot legs were relying on an ordering accident: the depletion usually lands while the *bot* is driving, where the manual wake bails on the missing park, rather than on the index-0 park between runs. Instant made the accident far likelier — a whole fork run drains inside one synchronous pump — and turned a latent race into a ~60%-reproducible hang. Both omsi bot legs now set the flag explicitly. **`omsi-multi-run-replay-retry` deliberately does not**, and the reason is measured rather than assumed: its pool's low-water mark across the replay is **5–10 of 350 and never reaches 0**, because omsi's run ends at its own ~350-mana budget and the reset rides that run end. The branch is unreachable, so the flag would be a control with nothing to control (8/8 green either way). Its exposure is in fact the *wider* one — a Playback park sets `_manualActionEntered` exactly like a hand-play park, so the depletion could land during the replay itself, not only on the index-0 park — which is why that leg now folds the low-water mark from the `manaChanged` mutations and asserts it stayed above zero. If an economy change ever eats the margin it fails immediately naming the pool, and *that* is the signal to add the flag.
...
**Recording vocabulary is now the SAME for all three fine-grained substrates.** jta (M4) converted at capture time to the **shared `actionQueue` vocabulary** (`clickTask` with `loops` = reps, `useItem` with `loops` = count) and replays through the shipped `jtaQueueEngine` executor — the option that generalizes; omsi (arc D) took exactly that dialect, so the prediction held; and the maze followed in slice **Q-b** (2026-09-02, on the user's ruling that a cross-substrate queue viewer is coming), retiring its native `{type:'move'|'wait'|'locationCheck', dir?, locationName?}` shape for `{actionType, actionId, substrate:'maze', loops}`. A maze `move` is `{actionType:'move', actionId:'N'|'E'|'S'|'W'}`, a `wait` carries `actionId: null`, and a `locationCheck` carries the location name as its `actionId`. Recordings compress runs of identical consecutive entries through `loops` (a 40-step corridor is one entry); the LIVE queue stays one entry per turn, so the panel's icon row still shows the press the player made. There was no on-read upgrade for stored maze recordings: the user ruled none were worth keeping, so `savedQueueStore.loadCache` DISCARDS a `substrate:'maze'` entry still carrying the old `type` key, naming how many on the console, and the store key stays `v1`.
