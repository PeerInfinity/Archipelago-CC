...
### R9 slice 10: ▶ CAMPAIGN — one click plays everything solved so far, and the page says where the playthrough stands

⚖ **Ruling 19** (user, 2026-08-21): *"One thing I'll want before we continue with
the existing plans is a way for the watch page to play the full sequence of
campaign tapes that we have solved so far."* Scoped, in the same ruling, to
*"the tapes that can be played continuously from a fresh game start"* and to
*"tapes generated by the solver"*.

**▶ campaign**, beside the queue box on `watch.html`. One click plays Seedling
from `new Game(0,80,128)` with an empty save to the L14 arrival — fifteen
windows, 3470 ticks, one game state — on both sides (the wasm arm still needs
the one human ▶ Start, by law).

**The button holds no chain id and no segment list.** It asks
`director.campaignChoice`, which picks by rule from `PAGE_CHAINS` and the new
`PAGE_CHAIN_META`:

| requirement | what it means |
|---|---|
| `custody` | every boot after the first is its predecessor's MEASURED latch, so the segments are one run and not a list |
| `true-start` | segment 1 declares nothing carried — no seam, no cleared set, no grant, no despawn |
| `solver-recorded` | every segment carries the solver's own decision-trace sidecar |

`PAGE_CHAIN_META` is a browser-safe copy for `PAGE_CHAINS`' own reason —
`playthroughWalk.js` imports `node:fs` and the sidecars are files — and
`director.test.js` recomputes every field of it from disk and reds on
disagreement. Two qualifying chains is a REFUSAL by name, not a tie-break.

⛓ **`true-start` is what the game means by it, not a coordinate.** Since ⚖ ruling
25 made the fork's boot reset unconditional (slice 9b), a tape that declares
nothing boots FRESH, so "declares nothing carried" IS "starts where a new game
does". `boot == {0, 80, 128}` would have been a claim about one room.

⛔⛔ **The solver requirement breaks a REAL tie.** `toy-west-pair` is also a
custody chain and `r7-ends-meet-1`'s boot block is BYTE-IDENTICAL to
`r8-solve-1`'s, so `custody + true-start` selects TWO chains today. The
true-start axis is kept anyway and its vacuity is named: it is a rule about
SCOPE — a custody chain that starts mid-game must never be offered as "play from
the beginning" — and the roster may hold one tomorrow.

⚠ **The sidecar predicate has exactly one roster-wide exception, and it is
pinned.** `diagonal-run` is a hand physics micro-fixture carrying a
`.trace.json` because it is `decisionTrace.test.js`'s FORMAT fixture; no
producer writes it. Over the domain the control applies the predicate to — chain
segments and headlines — it partitions exactly, in both directions, with both
sides non-empty.

**It is not a second player.** The click writes the chain id through the queue
control's own writer, so what runs is `?tapes=<chain>`: the same arm, the same
admission, the same LINK. It drops `?tape=`, because a campaign URL exists to be
sent to somebody.

#### THE CAMPAIGN READOUT — `#campaignReadout` / `window.__campaign`

Rooms crossed with a line each · the goal-ledger rows CREDITED · the end state ·
where it stopped · and the next work order.

The ledger is **measured**: `goalEarnedWitness` — `chainGoalFindings`' own
function — over segment k's boot and segment k+1's boot, which for a custody
chain IS segment k's measured latch. Today **2 / 41**: `sword@L10` at
`r8-solve-10`, `chest@L11` at `r9-solve-11`. ⛔ Deliberately not the model's item
mirror, whose own docblock warns that asserting an item from there is asserting
that a file agrees with itself. Only windows the run STEPPED are offered a
witness, so the readout reports the playthrough and not the roster.

Two things are **UNASSERTED BY NAME**: the last window (no successor boot, so its
latch is not on the page) and `seam.time` on the JS side (the model builds no
seam envelope; the wasm arm answers it from `botSeam()`).

⚠ **`window.__campaignControl` and `window.__campaign` are two names because they
are two states** — what the CONTROL knows before any walk (so a refusal is
readable on a page that has played nothing) and what a WALK produced. Published
under one name, a reader waiting for "the campaign readout exists" was satisfied
instantly by the pre-walk object on the page it had just left. Two states under
one name is a readout whose terminal condition cannot be written.

⛔ **Scope, as ruled.** The detached `r8-d2` tail is a real chain and is NOT
offered — not as a second section, not greyed out. The readout says the rooms
beyond the arrival are unsolved and names the refusal instead, and the gate
asserts that ABSENCE.

#### THE FRONTIER ARTIFACT — where it stops, and why

`frontend/modules/seedlingDemo/fixtures/campaign-frontier.json`, written by
`census-seedling-campaign.mjs --write-frontier` and kept honest by
`--check-frontier`. The census's `const STOP_STEP = 16;` is **gone**: the stop is
derived — the segments are walked and their `endLevel`s are the arrival sequence;
the chain covers the longest PREFIX of `route.steps` whose `crossesTo` sequence
equals those arrivals; the STOP is the first step after that prefix **the survey
refuses** (a step it SOLVES is a gap with a tape waiting to be recorded, not a
frontier). It reproduced the literal exactly — step 16, L14 → L15, family CAMERA
BAND — which is the point: the literal was right, and could not stay right.

The survey JSON is gitignored, so the artifact is the only thing a browser can
read. Its provenance is md5s of the survey's three declared committed SOURCES,
not a digest of `survey.json` — that file carries per-row `ms` timings and would
give a fingerprint that moves with the machine. `--check-frontier` runs four
structural rows anywhere and SKIPS the three survey-derived ones BY NAME where
the survey is absent.

#### THE GATES

`check-seedling-editor-sequence.mjs` **24 → 36 claims**. Claim 12 is a PRESS
path: it starts on a single-tape REPLAY view with no `?tapes=` in the bar,
clicks, and asserts what the click produced. Claim 13 re-derives every readout
value — never echoes one. `check-procgen-demos.mjs` **188 → 196**, with a new
`press` field that is actually CLICKED (`control` stays distinct and stays
unpressed: it names buttons whose effect a catalogue row cannot verify).

⚠ **A dead control is two different failures and only one was handled at first.**
A DISABLED button is read and named instantly. A button that exists, is enabled
and is wired to NOTHING made the gate throw an uncaught `TimeoutError` 300
seconds later with no named row at all — found by a mutant, fixed: the wait is
caught and becomes a row naming the button, and the rows that read the readout
are skipped by name.

⚠ **And a catalogue row that reads after a LOAD instead of a PRESS is blind.**
Measured: with the button present, enabled and wired to nothing, the load-path
row passed **completely green**.

#### A DEFECT IN THE SHARED INSTRUMENTS EXTRACTOR

`DOCUMENTED_FLAG_RE` used a CONSUMING trailing delimiter, which requires a
character after the flag name — and the last flag of a usage block has none. So
