...
|----------|-------------|
| [Architecture](./architecture.md) | Orientation: the pipeline, drivers, **level generation's two passes** (pass 1 = elements + connectors + the area graph, certified; pass 2 = the site-typed keep-or-revert loop), substrates, rules.json extensions, runtime, and Python round-trip. Read this first. |
...
| [Demonstrations — a catalogue](./demos.md) | Every demonstrable feature of the two procgen lab pages, one entry each: the link that shows it, the CLI command that reproduces it, which control to press and what you are looking at. ⛓ The catalogue is DATA — `frontend/modules/procgenDocs/demos.js` — rendered by [`demos.html`](https://peerinfinity.github.io/Archipelago-CC/modules/procgenDocs/demos.html) and IMPORTED by `scripts/procgen/check-procgen-demos.mjs`, which loads every link the page shows and asserts each entry's own claim. `demos.md` is the pointer at all three. |
| [Glossary — the vocabulary, defined](https://peerinfinity.github.io/Archipelago-CC/modules/procgenDocs/glossary.html) | **Start here if a word in any of these documents is doing work you cannot see.** 140 terms across the whole of procgen — the pipeline, the substrates, level generation, the lock-and-key layer, Seedling, the maze, the lab pages and the instruments — each with a plain-language sentence first and the concrete rule second. ⛓ The glossary is DATA — `frontend/modules/procgenDocs/glossary.js` — rendered by `glossary.html`, linked from every demo-catalogue entry's `terms:` line and from both lab pages' headers, and gated by `scripts/procgen/check-procgen-demos.mjs`. |
...
| [Maze Substrate](./maze.md) | The grid-of-tiles substrate: engine, biomes and wall backends, the action queue, content modules (hazards), the autopather, panel/editor — and the lab page's level generator: the area graph, the first element, and the `?areas=`/`?elements=`/`?require=` grammar. |
...
| [Seedling Real-Game Bot](./seedling-bot.md) | Driving the real recompiled Seedling with an input tape, and the differential that checks a JS physics transcription — movement, collision, room transitions and A\* pathing — against what the game actually did. Also carries the tracked record of the procgen arcs on `watch.html`, § *The procgen ELEMENTS design* being the current one. |
