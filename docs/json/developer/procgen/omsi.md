...
- **Exits are derived, not authored.** `_installRegionExits` walks `world.exits` (the same spiral-adjacency exits procgenPlayer routes on, exactly like jta's `_getRegionExits`). An exit is its `side` and its target, never a tile: the entry declares `regionGeometry: 'sides'`, so the pipeline writes no `x`/`y` on it ([Substrate Registry § *Build-time — region geometry*](./substrate-registry.md#build-time--region-geometry), PRESET SIDECARS G0). The walk injects one **synthetic exit action** per graph exit through `managed.js`'s `injectSyntheticAction`. Those actions are registered *after* `initializeActions()`, so they are resolvable through `getActionPrototype` (the queue's name lookup) but never appear in `totalActionList`, the planner census, or the DOM — vanilla enumeration and the byte-exact replay gate never see them.
...
Stepping is additionally skipped in three states: when the plan has no enabled runnable entry (every `singleTick` would `shouldRestart` and ping-pong resets with the host at 50/s), when the loops **step gate** is closed (below), and — during **live play only** — while the game's own start/pause control says stopped (below). Each refusal has its own counter in `getDebugState().clockStats` (`skippedNoQueue` / `skippedGated` / `skippedStopped` / `skippedHeldBoundary`), so a frozen substrate is attributable from a log instead of inferred from a flat tick count.
...
- **No-progress restarts** — a loop that consumed almost no effective time means nothing could run. Reporting it would ping-pong resets with the host over an unrunnable plan; instead the game idles locally until the player fixes the queue. Since the ⚖ 2026-09-06 ruling this guard is **player-visible**: the Play press that cold-starts live play calls `restart()` at the boundary the fork booted on, with `totals.effectiveTime` still 0 — so pressing Play costs the player no loop reset. The bridge logs the drop at DEBUG.
...
#### Live play also honours the game's own Start/Pause (⚖ 2026-09-06)

⚖ **RULED (user):** *"I want the restart to be triggered by the player pressing the in-game start button, not the addition of the first action of the queue. And in general, I want our code to respect the state of the in-game start and pause controls."*

An open step gate is **necessary and not sufficient**. `_mayStepClock` resolves which ARM opened the gate — `unenforced` / `replay` / `bot` / `live` — and on the **live** arm the bridge additionally requires the fork's own `gameIsStopped` to be false. The refusal is counted as `skippedStopped` and surfaced as `stepGateArm` / `forkStopped` / `liveStopped` in `getDebugState()`.

**Play is the cold start.** The fork boots stopped *and* holding a boundary (`saving.js`'s `load()` ends with a `pauseGame()` toggle; the compiled list is empty, so `shouldRestart` is set). The Play button is `onclick='pauseGame()'`, and `pauseGame()` calls `restart()` when it unpauses **at** a loop end — so one press clears both halves. Nothing on the host side cold-starts live play any more; before this ruling only the *instrument* did, by calling `IdleLoopsManaged.restartLoop()`.

⚠ **`pauseGame()` is a TOGGLE.** Read `gameIsStopped` before calling it — `if (gameIsStopped) pauseGame()`. `omsiSubstrateWrapper/test-helpers.js` exports `pressPlay()` / `pressPause()` / `isGameStopped()` that do exactly this; never write the flag directly, or the button handler's restart never runs.

**The exemptions, and why.** A **Playback replay** (`_replayInFlight`) and a **Bot walk** (`_stepGateBotRegion`) keep the bridge's timing: there the bridge and the fork's own planner own that flag between them (`resumeIfPlannerPaused` toggles `pauseGame()`, and `plannerPauseWhilePlanning` deliberately parks the game stopped between plans), and no Play press is coming because nobody is at the keyboard. The `unenforced` arm — loop mode off, or before the host's first gate push — is exempt too: no queue has parked there and no rule has claimed it.

**What the ruling does NOT couple.** The game's Pause does not pause loops' queue, and loops' park does not press the game's Pause. They are two controls over two things: the park says *which region may run*, the Play/Pause button says *whether the player wants time to pass*.

⚠ **Visible consequence of the fork's `pauseBeforeRestart` option.** With it on, the fork holds the boundary instead of restarting — the host clock then refuses to step (`skippedHeldBoundary`), and the run end is reported to the host **on the next Play press**, whose `restart()` fires `onRestart`. The host answers that report with a loop reset, and the reset **teleports** the player to the queue's start region. So under `pauseBeforeRestart` the player's Play press is what ends the previous run, and the reset is its visible answer.
...
⚠ **The predicate is a HELD BOUNDARY, not `stoppedAt`.** `load()` ends with a `pauseGame()` toggle, so `gameIsStopped` is **ambient-true throughout ordinary managed play** — folding it into *this* predicate would freeze every arm (240 ticks of real Wander progress were measured with `stoppedAt: true`). The stopped flag is a **separate, per-arm gate** — see [Live play also honours the game's own Start/Pause](#live-play-also-honours-the-games-own-startpause--2026-09-06) — and this one is about phantom loops, which a replay or a bot window mints just as readily as live play does. The correct test here is `timer >= timeNeeded` still true **after** a step batch returns: a legitimate crossing restarts inside the crossing tick, so post-batch persistence means something is *holding* the restart. Validated at 0 false positives across 1,600 batches / 481 real loops / 120,400 ticks, and it fires within 4 batches of a real planner pause. It is also planner-agnostic, which sidesteps `pausedByPlanner`/`awaitingPlan` being private to the fork's IIFE.
...
| `omsi-play-pause-controls-the-clock` | The ⚖ 2026-09-06 stopped gate, end to end: a parked region with a runnable plan and a STOPPED game refuses every step as `skippedStopped` (ticks flat, boundary held, no drain, no reset), Play is the cold start and costs no loop reset, Pause freezes the clock again and Play resumes it. It reloads the fork first, so it inherits no started game from the row before it |
