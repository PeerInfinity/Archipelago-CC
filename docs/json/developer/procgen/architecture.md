...
**275 instruments** live in `scripts/procgen/`, by prefix: `check-` 90 (54 browser) · `probe-` 60 (22 browser) · `plan-` 36 (1 browser) · `census-` 12 · `make-` 7 · `solve-` 7 · `sweep-` 6 · `dump-` 5 · `recon-` 5 · `region-` 5 · `generate-` 4 · no prefix 4 · `ci-` 3 · `extract-` 3 · `attribute-` 2 · `audit-` 2 · `export-` 2 (1 browser) · `measure-` 2 (1 browser) · `record-` 2 · `seedling-` 2 · `batch-` 1 · `build-` 1 · `derive-` 1 · `find-` 1 · `harvest-` 1 · `lint-` 1 · `mine-` 1 · `prove-` 1 · `reach-` 1 · `rerecord-` 1 · `run-` 1 · `shot-` 1 (1 browser) · `show-` 1 · `stamp-` 1 · `standing-` 1 · `survey-` 1.

80 of them drive a real browser; 189 accept at least one `--flag` OF THEIR OWN; 101 are cited by one of these documents; and 0 open with no comment at all.

Each also accepts what a module it IMPORTS parses: `--help` (273, in `argvHelp.js`) · `--wait-for-box` (100, in `boxLock.js`) · `--only` (1, in `rehearsalTree.js`) · `--record` (1, in `rehearsalTree.js`) · `--walk-report` (1, in `rehearsalTree.js`). Those are listed per row with the parse site named, so the table says what a file ACCEPTS without losing where the parse lives.
...
**The WORLD EDITOR is the maze lab's SET arm** (`mazeRoom/lab.html?source=set&world=<bundle>.zip`). ⛔ It is a `.zip` and not a JSON: a world NAMES its parts and holds no rooms of its own, so a bare `world.json` refuses by name and says where the document it names travels. Each part goes through its OWN validator first (`validateLevelSet`, `validateRegionLibrary`) and the world then binds them BY `doc_id` — never by position, because both documents are of the kind their part declares and a positional binder would load happily while every crossing landed on a door nobody drew. A world REPLACES a held library (its parts ARE the documents) and an overlay under a world refuses, because a world IS the composite overlay. The strip is one row of BOTH parts' rooms, each cell BADGED with the substrate that plays it — read off `readCell().substrate`, never derived, because a derived badge costs an atlas merge per paint and answers for regions the derivation may have dropped. ⚖ The one-renderer law holds: a maze cell goes to the maze's own painter and a Seedling cell gets a CARD naming the substrate, the room and its source kind, because `lab.html` may not import Seedling's painter.

**A room opens in ITS substrate's editor, and the dispatch is on the CELL.** A maze room opens on `lab.html`'s own canvas at the GLOBAL index, closing through the PART's own `closeRoomSession` against a session shim that re-globalises the op. A Seedling room opens in `watch.html`'s EDIT arm, hosted in an iframe on the page, through the room-editor contract — the `page` and the `arm` come off the substrate registry ENTRY, and the transport is a page-side one because `procgenLabPanel`'s host half needs a Golden Layout container, the app `eventBus` singleton and `iframeAdapterCore`, none of which a standalone page has. Two rooms of two substrates open at once is NOT supported and the strip refuses the second BY NAME before it opens. THREE sessions can therefore exist — the strip's, a maze room's, and a Seedling room's inside another page — and the page NAMES which one an undo will reach.

**A cross-part door is authored from the DERIVED exit ids**, not from either part's own vocabulary, and its DISPLACEMENT is shown before the press by appending the candidate link to a copy of the record and asking the merge what it displaced — one authority, and the sentence before the press is the sentence after it. The download is the world, both parts, each part's companion and the compile's `rules.json` + atlas, in ONE bundle with ONE stamp: `world_id` is minted at the press. Beside it sits an ALL-MAZE `rules.json` (M2) — every region compiled through the maze projection, which the player and the bots already walk — because a MIXED world does not play in the app yet: a door out of a Seedling room is the game's own synchronous level change and M1 is an AS3 slice. ⛔ It is a COMPILE-TIME projection: the region's authored `substrate` is stripped for that one compile so the compiler's BUILT-IN maze row runs, and nothing is written back. An INJECTED `sidecarBuilders` row cannot do this job — the maze row's context is built inside `compileRegionAtlas` from that compile's own graph and an injected builder never sees it.
