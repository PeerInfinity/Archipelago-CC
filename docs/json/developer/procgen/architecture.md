...
**The VANILLA 116 as an `xml` set, and why it is a FUNCTION rather than a fixture.** Everything above was demonstrated on generated rooms, because the committed `fixtures/seedling-vanilla-set.json` is 116 `embed`-sourced rooms and an `embed` cannot be opened. Two documents already in the repo are all it takes to have the real ones: the manifest (116 rooms with their names, music, `start`, `menu_rooms` and `named_rooms`) and `flashPanel/atlases/seedling-map.json`, the Phase-2 extract whose 116 records are exactly the shape `recordToOel` takes. `levelSetExporter.vanillaXmlSet(embedSet, mapDoc)` is the one writer of the result and `buildLevelSet` does the assembling, so there is no second exporter; the CLI's `--vanilla` arm and the page's `#editLoadVanilla` button are its two callers, and no third copy of the 116 is committed. **The join is by PATH, never by index** — a record's `path` relative to the map document's own `source.level_root`, matched against the room's `source.embed` on a segment boundary — and the measurement that makes that non-negotiable is that `levels[i].level === i` and `rooms[i].id === i` for all 116 today, so an index join agrees on this corpus and would carry every room's geometry under its neighbour's name the day either extractor's order moved. Every manifest field and every room field but `source` is carried verbatim and CHECKED by re-encoding, and `provenance.invented` coming back EMPTY is the proof no field was guessed.

**TWO `set_id`s DESCRIBE ONE GAME'S 116 ROOMS, and they coexist by construction.** The committed set keeps `seedling-vanilla-<hash>`: it is what the AS3 fork's `VanillaSet.SET_ID` names, what every save stamp keys on, and what the page's `?source=edit&level=N` ATLAS base checks. The derived set stamps base `seedling-vanilla-xml` and carries `provenance.derived_from` naming the other, so neither can be mistaken for the other and the page's identity line prints both. The xml id is PINNED as a measured literal in a node row: a change to `recordToOel`'s output or a re-extract of the map moves it BY NAME, which is what a derived document's id is for.

**⛔ AND THE VANILLA OVERLAY IS NOT A DOCUMENT.** What `make-seedling-playthrough-rules.mjs` adds on top of `deriveAtlas` — the reachability analyzer's sub-regions and internal exits, the lava-trap pulls, the hand rulings, `seedlingPlaythroughOverlay.js`'s entity semantics — is CODE, not the `{rooms: {i: {locations, rules}}}` document this session calls an overlay. So opening vanilla here opens it with an EMPTY overlay: every compiled edge is FREE, there are no locations, and the REPORT refuses the `rules.json` export — measured, and not only for the free edges: region `level_58` is UNREACHABLE from the start over the room data alone, because the only way into it in the real game is a mechanism no walk over transitions can see. The derived LAYER is nevertheless identical from both inputs: `deriveAtlas` over the map records and over the same rooms arriving as parsed OEL produce the same regions, the same boundary exits and the same connections, which is what carries the value round trip's 116/116 up to the atlas.
