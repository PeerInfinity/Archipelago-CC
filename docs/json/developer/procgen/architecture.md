...
**255 instruments** live in `scripts/procgen/`, by prefix: `probe-` 60 (22 browser) · `verify-` 49 (30 browser) · `plan-` 36 (1 browser) · `check-` 29 (22 browser) · `census-` 12 · `solve-` 7 · `dump-` 6 · `sweep-` 6 · `make-` 5 · `recon-` 5 · `region-` 5 · `generate-` 4 · `extract-` 3 · no prefix 3 · `attribute-` 2 · `audit-` 2 · `export-` 2 (1 browser) · `batch-` 1 · `build-` 1 · `ci-` 1 · `derive-` 1 · `find-` 1 · `harvest-` 1 · `lint-` 1 · `measure-` 1 · `mine-` 1 · `prove-` 1 · `reach-` 1 · `record-` 1 · `rerecord-` 1 · `run-` 1 · `seedling-` 1 · `show-` 1 · `stamp-` 1 · `standing-` 1 · `survey-` 1.

76 of them drive a real browser; 157 accept at least one `--flag`; 83 are cited by one of these documents; and 0 open with no comment at all.
...
| `mark-location` / `unmark-location` | `{room, entity: {type, x, y}, name, vanilla_item}` / `{room, name}` | the room holds no such entity at exactly those pixels (asked of `entities[]` for a record, of the text for an `xml` room); the name is not unique across the SET; the name is not marked |
...
**⛔ AND THE VANILLA OVERLAY IS NOT A DOCUMENT.** What `make-seedling-playthrough-rules.mjs` adds on top of `deriveAtlas` — the reachability analyzer's sub-regions and internal exits, the lava-trap pulls, the hand rulings, `seedlingPlaythroughOverlay.js`'s entity semantics — is CODE, not the `{rooms: {i: {locations, rules}}}` document this session calls an overlay. So opening vanilla here opens it with an EMPTY overlay: every compiled edge is FREE, there are no locations, and the REPORT refuses the `rules.json` export — measured, and not only for the free edges: region `level_58` is UNREACHABLE from the start over the room data alone, because the only way into it in the real game is a mechanism no walk over transitions can see. The derived LAYER is nevertheless identical from both inputs: `deriveAtlas` over the map records and over the same rooms arriving through a set (as records now, and as parsed OEL before that) produce the same regions, the same boundary exits and the same connections, which is what carries the value round trip's 116/116 up to the atlas.
