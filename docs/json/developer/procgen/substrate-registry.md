...
| Loop queue actions | move, check, explore | move, check (`executeVia: 'solver'`) | move, check (`executeVia: 'solver'`) | move, check, explore | move | move (`executeVia: 'solver'`) | move |
...
| Bot (solver, M6) | **yes** (delegation) | **yes** (`walkTo`) | **yes** (`walkTo`) | no | no | **yes** (`walkTo`, honors Instant) | no (deferred to arc D2) |
