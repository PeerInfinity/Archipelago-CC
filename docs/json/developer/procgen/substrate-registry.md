...
| `loopSupport.instant` | boolean | A Playback block can drain in one burst. Fine-grained: `replayActions` with `instant: true`; coarse-only: the generic executor honors the per-block flag. Gates the per-block Instant toggle. |
