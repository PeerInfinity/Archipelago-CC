...
Implemented by `maze` (the shared `adapterPrimitives.js` tile-grid implementations) and `text_adventure` (its own room hooks in `textAdventureRoom.js` since PRESET SIDECARS G2a — no tiles, and no rng draw):
...
**8 registered entries · 77 fields · 14 groups · 0 findings.** One column per entry the registry returns, one row per field an entry CARRIES — `substrateRegistry.getAll()` for the columns and `Object.keys(entry)` for the rows, so a field a substrate grows appears here without anybody editing a table.
