...
| `sidecarFields` | `{ [payloadKey]: descriptor }` | **Declares what `serializeWorld` writes** — one descriptor per top-level `playable_payload` key: `{type, required?, enum?, derived?, description, schema?}` (a JSON type; present on every entry its producers write; a closed value set; COMPUTED at write rather than authored — its description then names the writer; what the field is; an optional JSON-Schema fragment for nested shape). Every substrate with a `deserializeWorld` declares one (PRESET SIDECARS D0). |
...
**8 registered entries · 66 fields · 12 groups · 0 findings.** One column per entry the registry returns, one row per field an entry CARRIES — `substrateRegistry.getAll()` for the columns and `Object.keys(entry)` for the rows, so a field a substrate grows appears here without anybody editing a table.
