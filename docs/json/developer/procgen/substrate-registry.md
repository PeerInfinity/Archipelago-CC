...
| `zoneConfigFromSlot({entries, locations})` | **Read back, from ONE slot of a document, the config its zones were built with** — without installing anything (APWORLD SUBSTRATE CHANGE R5b). `entries` = this substrate's sidecar entries by region; `locations` = region → `[{name, item, access_rule}]` over the slot. Answers `{ok, cfg, assumed, zoneCount, host}`: `cfg` is what the document records, and `assumed` is what it does not (the setters' own defaults). The hub VERIFIES `assumed` by re-extracting every committed zone of the slot, and refuses the slot by name when one does not reproduce. Declared by **jta**: the dataset from the sibling that hosts `jta_dataset`, `emitZoneLocations` from `ap_locations`, and `goalZone` from the perk universe a zone rule names (else the Victory placement; with the zone locations on and neither left, it REFUSES — `null` would silently drop the goal), with `perkShuffleSeed`/`freeZones`/`startingPerks` assumed. An entry that declares the zone channel but not this hook (bounce, runner, omsi, flash_seedling) gets no *Zone N* source: the op refuses it by name. |
| `zoneOfPayload(payload)` | **The zone a region's payload plays**, or `null` — the hub's "held by" test (a zone another region of the slot already plays is refused, naming that region). Declared by **jta** (`jtaZone`). |
