...
| `deserializeWorld` | `(playable_payload) → world` | Called by procgenPlayer per region when building the warehouse. **Shape requirement:** the returned world's `exits` must be a `Map` keyed by exit name — `procgenPlayer.handleRegionMove` calls `world.exits.has(exitName)`; an array breaks region transitions. **Flag requirement:** a payload's `manaEnabled: true` must be `true` on the returned world — `procgenPlayer.getRegionInfo` reads it off the world, never the payload (`sidecarFieldsRegistry.test.js` holds every entry to it). **A refusal is a throw, and it costs one region, not the document** (PRESET SIDECARS C1): a payload that is not the entry's shape may be refused by throwing a sentence; the three readers that build worlds for a whole document catch it through `procgenCore/deserializeRefusal.js` — the warehouse SKIPS the region (logged, kept on `warehouse.refused`), the composite map SKIPS it (`refused` on the result, `refusedRegionsNote` on the Map tab and the pipeline's message), and the sphere rebuild, which cannot drop a node of its tree, refuses the document by `SPHERE_REBUILD_REFUSALS.unreadablePayload` (`deserializeRefusal.test.js`). |
...
| `sidecarFields` | `{ [payloadKey]: descriptor }` | **Declares what `serializeWorld` writes** — one descriptor per top-level `playable_payload` key: `{type, required?, enum?, derived?, description, schema?, references?}` (a JSON type; present on every entry its producers write; a closed value set; COMPUTED at write rather than authored — its description then names the writer; what the field is; an optional JSON-Schema fragment for nested shape; and, since V0, `{field, key}` when the value POINTS AT A SIBLING ENTRY — its `key` must equal a same-slot entry's `payload[field][key]`). Every substrate with a `deserializeWorld` declares one (PRESET SIDECARS D0). |
| `apLocationNamesOf` | `(playable_payload) → string[] \| null` | *(optional)* **Where the payload carries its AP location names** — `null` when this payload carries none. The hub's validity report holds them to the document's region both ways. Absent = location names are NOT CHECKED for this substrate, said aloud (PRESET SIDECARS V0). |
| `apExitNamesOf` | `(playable_payload) → string[] \| null` | *(optional)* **The payload's exits as the region's COMPLETE exit list** — `null` when the payload has none. A document exit missing from it is reported. Absent = the substrate's producer writes only some of the region's exits, and the check is NOT RUN, said aloud (PRESET SIDECARS V0). |

⛓ **The payload declaration** (PRESET SIDECARS D0, 2026-09-10; ⚖ Q1 C). `rules.schema.json` keeps `playable_payload` OPAQUE — the payload is the substrate's, and its serializer is the authority — so the shape is declared HERE, on the entry, beside the serializer it describes. The vocabulary is `procgenCore/sidecarFields.js`: `validateSidecarFields` checks a declaration's shape, and `sidecarFieldsOf(entry)` is what a reader asks — the entry's own descriptors merged with the **envelope**. ⛔ The envelope is four keys the ENGINE writes on every payload, whatever the substrate: `buildPresetSidecars` re-attaches `exits` (and `entrance`, when defined) before calling `serializeWorld`, and stamps `manaEnabled` (loop mode only) and `fogEnabled` after it. They are declared ONCE, in `ENVELOPE_SIDECAR_FIELDS`, and a substrate that redeclares one is refused by name (`ENVELOPE_REDECLARED`) — the one thing it may say about an envelope key is that its producers always write it, spelled `REQUIRED_ENVELOPE_FIELD`. `maze` carries its declaration beside `serializeMazeWorld` (`mazeRoom/mazeSerializer.js`), and `text_adventure` its OWN beside its room serializer (`textAdventureSubstrateWrapper/textAdventureRoom.js`: `exitGates` and `locations`, no tile key — until PRESET SIDECARS G2a the two carried one object); the flash-zone family shares: `bounce` and `runner` build theirs from `flashZoneSidecarFields` (`flashSubstrateLibrary.js`) with their own level key. ⛓ **The gate is `scripts/procgen/check-sidecar-fields.mjs`**: every tracked `preset_sidecars` entry is checked against its substrate's declaration — an UNDECLARED payload key fails (that is the pin between declaration and serializer), as do a missing required key and a value outside its descriptor — through `sidecarPayloadErrors`, the same predicate an editor's validity report asks. ⛔ Like `roomEditor`, it is a declaration and not a registration: `register()` validates only `id` and `sharing`, and the `shared/` submodule is untouched.

⛓ **The AP-name carriers and the validity report** (PRESET SIDECARS V0, 2026-09-10). `apworldEditor/sidecarIssues.js` — the APWorld hub's validity report, which the validation bar, the per-region block and `check-sidecar-fields.mjs`'s second layer all read — checks a payload against the rest of the DOCUMENT, and it names no substrate: what differs per substrate is read here. `apLocationNamesOf` says where the AP location names live — the tile-grid payload's `items[].locationName` (`tileGridApLocationNames`, beside `serializeMazeWorld`; `maze`), the room payload's `locations[].name` (`textAdventureApLocationNames`; `text_adventure`, since G2a), the VALUES of `ap_locations` for the flash family (`createFlashSubstrateEntry`'s default, so `flash`, `bounce` and `runner`), `jta` and `omsi` (`nameMapValues('ap_locations')`, in `procgenCore/sidecarFields.js`). `apExitNamesOf` is `envelopeExitNames` wherever the engine's re-attached exit list is the payload's — every one of those. ⛔ `flash_seedling` REMOVES both of the factory's defaults: the atlas compiler writes no `ap_locations` (a Seedling region's locations come from the atlas graph), and its `exits` are the teleporter doors only — a walk crossing between two sub-regions of one level is geometry. Absent, the report says one warning per slot that it did not check. The generic `references` descriptor key replaced a jta-only check: jta's `jta_dataset_ref` declares `{field: 'jta_dataset', key: 'dataset_id'}`, the same match `buildWarehouse` makes at play.
...
| `compositeMap.drawRegion` | `(ctx, region, { offX, offY, regionSize, tilePx, colors })` | **The painter**, called once per placed region with the cell's origin in canvas pixels and the shared geometry. Declared by **maze** (`mazeRoom/mazeCompositeMap.js`) and **text_adventure** (`textAdventureSubstrateWrapper/textAdventureCompositeMap.js`, which since G2a paints the deserialized ROOM — exits by side, locations and their authored gates — and reads no payload key of its own), each carrying the imports that make its painter substrate-specific. |
...
⛓ **`exitSides` — why a side move is a relabel, and who says what it relabels.** For a flash zone an exit's `side` is a LINKING KEY: play resolves `params.sidePortals[exit.side]` to a level portal (`flashSubstrate/bridge.js`; the bounce page inverts it to portal → side), and the portal's geometry does not depend on the side. So an exit moved to another side is its `side` plus every payload fact keyed by it — and which facts those are is the substrate's to say, so the hub names none of them. The flash-zone family's shared half is `flashZoneExitSides` (`flashSubstrateLibrary.js`): `params.sidePortals` re-keyed IN PLACE (the key keeps its position, so a move and its move back are byte-identical — and since PIPELINE RELAYOUT R2 the Procgen Pipeline's own Move Exit, `moveSphereExitSide` / `swapSphereExitSides`, runs this same declared relabel instead of re-keying the map by hand), and `params.backExitSide` (the entrance side the engine stamps for the back-portal route) moved with the region's back exit and never added. Bounce adds its portal's `direction` arrow (`BOUNCE_EXIT_SIDES`), which `sideExits.portalIdsBySide` reads FIRST when a level portal's side is asked. Runner adds nothing: its level portals' `arrow` follows the platform, not the side (measured: `exit_br0` on side S carries `arrow: 'up'`). Like `regionGeometry`, it is declared on each zone game's ENTRY, never as a `createFlashSubstrateEntry` default (`flash` / `flash_seedling` carry no portal map). `text_adventure` (PRESET SIDECARS G2a) declares the slot with an EMPTY `keys` list and an identity relabel (a clone): its side is where the compass lists an exit, and its gates are keyed by `exit_id`, so nothing else moves — and an empty list is a declaration, while only an absent slot refuses. jta and omsi carry `side` too, but only LABEL an exit by it (measured at M3 over every tracked jta and omsi sidecar: no side-keyed map and no side-valued field outside `exits`); since PIPELINE RELAYOUT R2 (⚖ the user) they declare the same thing. All three declare ONE object, `SIDE_AGNOSTIC_EXIT_SIDES` (`procgenCore/exitSides.js`), never a copy of it; omsi's synthetic exit action is keyed by the exit's `exitName` so two exits on one side never collide in the fork (`omsiSubstrateWrapper/exitActions.js`). The maze declares nothing on purpose: its exit IS a tile in the payload's grid, so moving it to another side is a geometry edit (the room editor's), not a relabel — every exit-side editor refuses it by that absence.

⛓ **A second exit on a side — decided by the declaration** (PIPELINE RELAYOUT R2). "One exit per side" is not a law of the tree: the committed maze corpus holds regions with many exits on one side, and the text adventure's compass numbers them. It is a law of a PAYLOAD that keys a fact by side. So every exit-side editor asks one function, `sideMayHoldAnotherExit(entry)`: a side that already carries an exit may take another **iff the entry declares `exitSides` with an EMPTY `keys` list** (nothing in the payload collides); a keyed declaration (the flash-zone family's `params.sidePortals`) says no, and the editors keep their refusal (*"… moving X there is a swap"* in the hub, *"side S already has an exit"* in the pipeline); an absent or malformed declaration says no for that absence. No substrate name is consulted.
...
| `regionRoundTrip` | object | **Declares how a region of a `rules.json` DOCUMENT goes into that editor and comes back** (APWorld editor hub, H4b). `roomEditor` says WHICH editor; this says what it wants handed in and how to read its save. `open({regionId, payload, region, itemPool, expectedItems})` returns `{session, unedited}` — `session` is spread into `roomEditor.open()` beside `onSave`, and `unedited` is the value that editor's save would carry for a session nobody touched. `save(saved, ctx)` returns `{payload, exits, locations}`, the re-derived sidecar payload and the compiled endpoints (`compileRegion`'s shape; a `name` on an endpoint is the DOCUMENT's own name when the substrate knows it, as bounce does from `ap_locations`). Either member may be async. `{refused: '<why>'}` instead of the two declares that there IS no document round trip and says why in the substrate's own words — Seedling's payload is an atlas reference, not a room record. Optional `rules` (PRESET SIDECARS G2b-1; vocabulary `procgenCore/roundTripRules.js`, read by `roundTripRulesOf`, a value outside it refused by name) says where `save`'s rules come from: `'authored'` — the payload carries them and they must equal the document's (`text_adventure`, the only declarer); `'derived'`, the default — re-derived from geometry (maze, bounce). `check-sidecar-fields.mjs`' third layer fails an authored disagreement and counts a derived one. Declared by `maze`, `bounce` and `text_adventure` (`textAdventureRegionRoundTrip.js`: no library, no re-stamp — the room carries its identity; the engine-stamped envelope keys re-appended), refused by `flash_seedling`. `apworldEditor/regionRoundTrip.js` resolves it and names no substrate. |
| `exitSides` | object (optional) | **Declares what ELSE in this substrate's payload is keyed by an exit's `side`, and how to relabel it** (PRESET SIDECARS M3). `{keys, relabel}`: `keys` names the payload paths the relabel rewrites (words for a reader: the docs, the op's answer); `relabel(payload, moves)` is PURE (clone in, clone out) and takes `moves = [{exitId, from, to}]` applied SIMULTANEOUSLY (one for a move, two for a swap). It never writes `payload.exits` itself: the APWorld editor's `move-exit-side` / `swap-exit-sides` write those through the substrate's own `deserializeWorld` → `serializeWorld`. ABSENT ⇒ the ops REFUSE, in the declaration's words (*"its registry entry declares no `exitSides`, so the hub cannot say what else in the payload is keyed by side"*); a malformed one is refused by name. `procgenCore/exitSides.js` (`exitSidesOf`) is the one reader; `sideMayHoldAnotherExit(entry)` there is the one occupied-side rule (a second exit on a side iff `keys` is empty — PIPELINE RELAYOUT R2), and `SIDE_AGNOSTIC_EXIT_SIDES` the one keyless declaration. The APWorld editor's ops and the Procgen Pipeline's `moveSphereExitSide` / `swapSphereExitSides` (through `regionExitSides(region)`) both read it. Declared by **bounce**, **runner**, **text_adventure**, **jta** and **omsi** — derived, not listed: `sidecarFieldsRegistry.test.js` asserts that every `'sides'`-geometry entry declares the slot and no `'tiles'` entry does, that every entry whose `sidecarFields` declare `params.sidePortals` declares it, and that `keys` names the portal map exactly when the payload declares one (a keyless declaration's relabel is an identity clone). |
...
Implemented by `maze` (the shared `adapterPrimitives.js` tile-grid implementations), `text_adventure` (its own room hooks in `textAdventureRoom.js` since PRESET SIDECARS G2a — no tiles, and no rng draw) and, since SEEDLING GENERATED LEVELS G1 (2026-09-23), `flash_seedling_gen` — a room the Seedling generator builds to the region's spec, its hooks in `seedlingDemo/seedlingGenRoom.js`, reached through the entry's install seam (`installSeedlingGenRoom`, so the flash panel's static closure never reaches the generator; [Flash Substrate § *Generated rooms*](./flash.md#generated-rooms-flash_seedling_gen)):
...
Today's content sources are the zone-based substrates (`jta`, `bounce`, `runner`, `omsi`) and, since SEEDLING IN THE PIPELINE T1 (2026-09-23), `flash_seedling` — the first content source whose entries are REAL rooms of an existing game: each is one AP region of a region atlas, placed as the real level and entered through its real doors ([Flash Substrate § *As a content source in the pipeline*](./flash.md#as-a-content-source-in-the-pipeline)). Their registry adapter exposes:

| Field | Meaning |
|-------|---------|
| `zoneCount` | **Pool size** — how many discrete entries exist. Layout drivers (currently `arrangeShuffledSpiral`) refuse to allocate more regions than this to the source, and the quota-vs-pool check keys on it. Since APWORLD SUBSTRATE CHANGE R5b it has a second consumer: the APWorld hub's *Zone N* source (`apworldEditor/regionContent.js`), which draws a picker over `0..zoneCount-1` — read off the document through `zoneConfigFromSlot` (below), never off an install. |
| `extractZoneRules(zoneIdx, ctx)` | **Instantiate** — the single per-ordinal content channel: produces the entry's locations, per-side exit rules/paths, obstacle defs, and `playable_payload` fragment in one call. jta folds its `{ jtaZone: zoneIdx }` sidecar ordinal into this channel's payload (region-library C1 absorbed the former standalone `synthesizeZonePayload` hook — `jtaZone` stays the first payload key); bounce/runner emit their winnable geometry's locations and rules. The hub's `replace-region-content` op (R5b) runs it in its generation WORKER, under the config the document records, and inlines the answer in the op, so a refold never installs. |
...
| `applyPipelineConfig(cfg)` | **Install this source's pipeline config** — the ① arrange step calls it through `applySubstrateConfig` so the quota-vs-`zoneCount` check sees a configured dataset's real zone count. Every field defers to its setter's own default, so `applyPipelineConfig({})` resets the vanilla path exactly. Declared by jta, omsi and `flash_seedling` (whose `{atlasDoc}` defaults to the starter atlas, so a preset — which carries no `substrateConfig` — places starter-atlas rooms). ⛓ Documented in full one door down, at its consumer: [The Stepped Pipeline § *Spiral mode — four steps*](./stepped-pipeline.md#spiral-mode--four-steps). The hub's *Zone N* source (R5b) is its second consumer: it installs `{...cfg, ...assumed}` from `zoneConfigFromSlot` in the generation worker only, because the install is module-global and the page's registry is the pipeline panel's too. |
| `zoneConfigFromSlot({entries, locations, blocks, fetched})` | **Read back, from ONE slot of a document, the config its zones were built with** — without installing anything (APWORLD SUBSTRATE CHANGE R5b). `entries` = this substrate's sidecar entries by region; `locations` = region → `[{name, item, access_rule}]` over the slot; since R5c also `blocks` = the document's top-level blocks this entry writes (keyed by its own `rulesJsonBlocks()` answer, a missing one present as `undefined`) and `fetched` = `{[path]: doc}` of the served documents already fetched. Answers `{ok, cfg, assumed, zoneCount, host}`, optionally `zoneNames` (the picker lists a zone by name) and `unplaceable` (`[{name, why}]`, listed disabled). `cfg` is what the document records, and `assumed` is what it does not (the setters' own defaults). The hub VERIFIES `assumed` by re-extracting every committed zone of the slot (with each region's own exit sides, comparing the SERIALISED entry), and refuses the slot by name when one does not reproduce. A read-back that needs served documents answers `{ok: false, needs: [path]}`; the caller (`resolveZoneFetches`: the generation worker, the page's picker, the control's disk read) fetches them and asks again, and a failed fetch is a refusal naming the path. Declared by **jta**: the dataset from the sibling that hosts `jta_dataset`, `emitZoneLocations` from `ap_locations`, and `goalZone` from the perk universe a zone rule names (else the Victory placement; with the zone locations on and neither left, it REFUSES — `null` would silently drop the goal), with `perkShuffleSeed`/`freeZones`/`startingPerks` assumed; jta ignores `blocks` and `fetched` (a row pins its answers byte-identical with and without them). Declared by **`flash_seedling`** (R5c): the atlas named by `blocks.region_atlas.atlas_id` — the bundled starter needs no fetch, any other is resolved through the served index `flashPanel/atlases/atlas_files.json` and fetched — with `zoneNames` = the placeable rooms' AP names and `unplaceable` = the doorless ones; refused by name: a missing block, a room whose `atlas_ref` is not the document's atlas, the atlas COMPILER's projection (exits that are the level's own transitions, not doors bound to sides), an id the index does not list, a fetched atlas of another id. An entry that declares the zone channel but not this hook (bounce, runner, omsi) gets no zone source: the op refuses it by name. |
| `zoneOfPayload(payload, cfg)` | **The zone a region's payload plays**, or `null` — the hub's "held by" test (a zone another region of the slot already plays is refused, naming that region). `cfg` is the read-back's config (R5c): a room's ordinal is its index in the ATLAS, which the payload names only by room. Declared by **jta** (`jtaZone`; ignores `cfg`) and **`flash_seedling`** (`atlas_region` + `atlas_sub_region` matched in `cfg.atlasDoc`'s placeable rooms; `null` without the config or for another atlas). |
| `zoneSourceLabel` | **The hub's word for this entry's zone source** on the Region generation form's Source row and picker (APWORLD SUBSTRATE CHANGE R5c), so no substrate is named in the hub. Absent = *Zone N* (jta). `flash_seedling`: *Atlas room*. |
...
| Field | Type | Meaning |
|-------|------|---------|
| `regionGeometry` | `'tiles'` \| `'sides'` (optional) | **Declares whether this substrate's regions stand on a tile grid or are only SIDES.** `'tiles'`: each exit stands on a tile, so the engine writes `exits[].x/y`, `exits_placed[].tile_position` and `extracted_rules.exits[].position` (the maze's spawn, BFS and serializer read them). `'sides'`: a zone whose exits are only their sides, so the engine writes **none of the three**, and no `entrance` tile either. ABSENT ⇒ `'tiles'`, which is what every entry got before the slot existed. Declared `'sides'` by **jta** and **omsi** (PRESET SIDECARS G0, 2026-09-11) and by **bounce** and **runner** (G1, 2026-09-11) — the four zone games, the entries whose rules come from `extractZoneRules` — and by **flash_seedling** (SEEDLING IN THE PIPELINE T1, 2026-09-23 — a zone game now, too: `extractZoneRules` places a real room), and by **text_adventure** (G2a) and **flash_seedling_gen** (SEEDLING GENERATED LEVELS G1), PROCEDURAL entries whose own `generateRegionCore` places no exit on a tile (`sidecarFieldsRegistry.test.js` derives the population: the zone games plus every entry whose core, driven once, places none). |
...
⛓ **The one cross-boundary read, and its fallback.** A tile region's neighbour sometimes MIRRORS one of its exit tiles. A maze child's entrance is its parent's exit tile mirrored across the shared side (grid-growth, top-down and sphere each do this once), and a reciprocal back-exit is the source exit's tile mirrored (`reconcileBidirectionalExits`). A sides-only parent stores no tile, so the engine mirrors `perimeterMidpoint(side, regionSize)` instead. That is exactly the tile a zone region carried before this slot existed, so **a maze child's bytes do not move**. The sphere driver's throw at a parent with no `exits_placed` entry on the child's side cannot fire for a sides-only parent: the entry is still written, just without a `tile_position`. `procgenPipeline/regionGeometry.test.js` flips one entry's declaration through a registry spy and diffs the two worlds built from one seed (spiral jta with its declaration masked; sphere and top-down with bounce spied back to `'tiles'`, the world before G1). Every difference is the removal of one of those fields, on regions of the flipped substrate only, and every maze region is byte-identical. The same file drives the sphere and top-down worlds over the REAL registry, where bounce is a sides-only parent of a maze child.

⛓ **Who reaches which branch.** jta and omsi reach the engine only through the spiral's content-source path (`assembleZoneRegion`): neither has a zone realiser, so the sphere and top-down drivers refuse them by name (*"neither generateRegionCore nor generateZoneForSpecs"*), and grid-growth refuses zone substrates outright. bounce and runner DO have one (`generateZoneForSpecs`), so since G1 a sides-only region — and a sides-only PARENT of a maze child — arises in the sphere and top-down drivers for real; the regions they realise carry no tile field and no `entrance`. An exit-side MOVE (`moveSphereExitSide` / `swapSphereExitSides`, the pipeline's Move Exits, through `relabelExitSide`) asks the same law: a sides-only region gets its new `side` (world exit and `exits_placed`) and no tile, and a tile it still carried — a document written before its substrate declared sides — is REMOVED rather than left on the side it left; a tiles region still gets the side's midpoint tile, the slot the renderer draws a single exit in. ⚖ `regionLibraryValidator.js`'s `LIBRARY_V1_SUBSTRATES` is a HAND LIST of the same split (procedural vs content), and it is deliberately NOT retired onto this slot (⚖ planner, G1): the validator runs where nothing is registered, and there a registry read would refuse the bounce and runner packs (§ *Build-time — region library entries*). Like `roomEditor`, the slot is a declaration and not a registration: `register()` validates only `id` and `sharing`. The validity check is `geometryOf`, asked of every entry by `procgenCore/sidecarFieldsRegistry.test.js`.

### Build-time — generation cost

| Field | Type | Meaning |
|-------|------|---------|
| `generationCost` | `'light'` \| `'heavy'` (optional) | **Declares how expensive this substrate's regions are to generate headless.** `'heavy'`: a region is found by generate-and-test, seconds apiece, so a Procgen Pipeline preset that names the substrate (a quota, a mix weight or its start substrate) is NOT generated by the CI preset row `procgenPipeline/presetDefs.generate.slow.test.js`; that row prints the skip as a sentence and requires the skipped set, derived from this field, to equal `PRESETS_SKIPPED_AS_HEAVY` in `presetDefs.js` — so a preset cannot join the skip silently (⚖ user 2026-09-16: *"Skip runner presets in CI, by a declared field"*). Such a preset is still held to `PRESET_HEADLESS_BUDGET_MS` (30 s) by hand and by the box's browser gate. ABSENT ⇒ `'light'`; a value outside the two is refused by name. Declared `'heavy'` by **runner** (its two 3-region sphere presets measured 6.1–26.7 s headless across 1-minute loads of 1.3–11.8). `procgenPipeline/presetRun.js` (`substrateGenerationCost`, `heavySubstrateIds`) is the one reader. |
...
| `instantiateLibraryEntryForSpecs(entry, ctx)` | **Instantiate, requirement-aware** (region-library F6a/F6c) — the sphere-placement variant: it relabels the captured openings/portals onto the slot's child sides and reassigns the node's items, and the engine overlays each child gate as an `access_rule`. **Two consumers:** the sphere path (`procgenPipelineEngine.buildSphereLibraryRegion`) and, since APWORLD SUBSTRATE CHANGE R5a, the APWorld hub's *Library entry* source (`apworldEditor/regionRegenerate.js`, the `regenerate-region-sidecar` op with `source.kind = 'library'`). The hub passes the same `ctx`, but for a tile entry the document's exit to the parent is a child side too, since the hub has no back-portal pass. It then stamps the document's location names onto the slots k-th onto k-th and drops the surplus slots. Declaring the hook is also what draws the hub form's Source row for the substrate. |
| `libraryEntryRefusal(entry, ctx)` | **The requirement-aware hook's refusal, asked BEFORE it runs** (APWORLD SUBSTRATE CHANGE R6) — the sentence `instantiateLibraryEntryForSpecs` would throw for this `ctx` because the entry captured too few openings (maze: its `payload.exits` against the ctx's CHILD sides — slot 0 is the entrance) or portals (bounce: its `sidePortals` against every side), else null. One rule in one wording: maze's hook builds its throw with the same sentence builder, bounce's hook calls the refusal itself. **Consumer:** the APWorld hub's `regenerate-region-sidecar` op asks it (over the same `ctx` it hands the hook) before the hook runs, so the *Library entry* picker disables such an entry in the hook's own words instead of offering it. Declared by maze and bounce; runner (retiring) declares none, and its hook still refuses at the press. Knob-dependent refusals (`mazeRequireSameWall`, `mazeRequireTileAlign`) stay the hook's. |
...
The sphere-growth driver and the Procgen Pipeline panel read a further set of optional hooks so the generic engine never names a substrate directly. **Bounce and runner implement the zone-generation and gate hooks** (runner all but `canHostExitGatesBraid`, `driftItems` and `prepareSphereGrowth`); **the maze implements only panel-integration hooks** — `defaultProcgenParams`, `renderProcgenParams` and the library-scoped pair below, no `buildRegionParams` (the maze document's *Registry entry* section says why). The generated matrix below is the authority on which entry carries which. (This sentence said *"today only bounce implements them"* until 2026-08-18, when generating the matrix from the entries showed otherwise; the hand-kept matrix it replaced said the same thing on its "Sphere-growth adapter hooks" row.) The semantics are documented at their consumers in `procgenPipelineEngine.js` / `sphereConfigHooks.js`:
...
- **Pipeline panel integration:** `defaultProcgenParams`, `prepareSphereGrowth`, `buildRegionParams`, `renderProcgenParams` (per-substrate parameter defaults/controls, consulted for the substrates the run's quotas or mix select; `renderProcgenParams` is drawn through the shared per-region generation form, below, which is its one caller). Two more are scoped to a region-library SELECTION instead: `buildLibraryRegionParams` (the regionParams a selected library entry of this substrate reads) and `renderLibraryProcgenParams` (their controls, in the panel's Region libraries subsection), consulted for the substrates the selected sphere libraries' entries realise — which need no quota of their own. The maze declares that pair for its packs' connection-strictness flags (`mazeProcgenParams.js`). `procgenParamsFromPayload(payload)` is the inverse direction: the partial params bag an EXISTING payload records about how it was generated, so a per-region generation form opens on the region's own knobs instead of the defaults (`procgenCore/regionGenerationForm.js` `bagFromPayload` overlays it on `defaultProcgenParams`). Bounce and runner declare it, each reading only the physics profile its payload stamps (`params.physics.profile`; an absent bounce stamp means `experimental`, which stamps nothing); nothing else they generate with leaves a readable trace in the payload, and the maze's payload records none of its knobs.
...
- **Starting inventory:** `startingInventory` — DATA, `{needs: [{anyOf: [item…], reason}]}`: what a world built of this substrate's regions needs in its starting inventory, each need met when the starting list holds any one of `anyOf` (apworld substrate change R3). Bounce declares one need, *one of* `Left arrow` / `Right arrow` — the same fact `prepareSphereGrowth` acts on when it grants an arrow at the start (a bounce level cannot gate both arrows in one region, and hosts at most one arrowless-gated exit); no other entry declares it (runner has no `prepareSphereGrowth` and no drift items). Read by `procgenCore/startingInventory.js` (`startingInventoryNeeds(entry, starting)` → `[{anyOf, reason, met, heldName}]`), whose consumer is the APWorld editor's **Starting inventory** block (the Items tab, and the Region generation form for the target substrate — `docs/json/modules/apworldEditor.md`). `procgenCore/startingInventory.test.js` holds every entry's declaration to names the entry itself carries (`libraryItems` ∪ `driftItems`), and holds what `prepareSphereGrowth` grants to meeting every declared need.

| slot | kind | declared by | consumer |
|---|---|---|---|
| `startingInventory` | data | bounce | APWorld editor — the Starting inventory block (Items tab) and the Region generation form's need line; `startingInventoryNeeds` |

**The per-region generation form** (`procgenCore/regionGenerationForm.js`, apworld substrate change R1) is where a region's generation settings are drawn, for any host. `renderRegionGenerationForm({substrateId, params, onChange, generic, seed})` draws an optional seed row, the generic per-region rows (`REGION_GENERATION_FIELDS`: region width and height, for an entry whose `regionGeometry` is tiles only, and max items per region), then the entry's `renderProcgenParams` node under a *"<id> parameters"* subheader; an entry without the hook gets no subheader, and the form's `data-procgen-params` attribute says `none`. It names no substrate and imports no panel, and the one `fieldRow` / `numberField` the three hooks build with lives there. The Procgen Pipeline panel's **Parameters** section is its first host: the number grid keeps the per-WORLD rows (seed, grid size outside the spiral and sphere modes, the sphere counts) and draws the generic per-region rows in their old place, the Region XP effect, grid-growth toggles and loop-mode rows follow, and then one form per active substrate is drawn with `generic: false`. The params bag, its keys and defaults, `buildRegionParams`, `effectiveHazardOpts` and every run path are the same as before the split; `procgenPipelineUI.test.js` holds the section's DOM and bound keys against fixtures captured before it. The APWorld editor's region block is the second host (R2); `bagFromPayload` gives it a bag that starts from the region's own payload.
...
| `text_adventure` | The surviving wrapper (the direct-panel `textAdventureSubstrate` was deleted 2026-07-26); its `loopSupport` is pinned by `textAdventureSubstrateWrapperLibrary.test.js`. Playback controller is a host proxy → iframe bridge, publishing `textAdventureSubstrateWrapper:control`. **Coarse-only** capture: no recorder and no saved actions — the block's own queue entries *are* the recording (since M4 with an actions-less saved entry for the economy metadata). Its build hooks are its own room (`textAdventureRoom.js`, PRESET SIDECARS G2a); until G2a they were the shared `adapterPrimitives.js` tile-grid implementations. |
...
| `flash_seedling` | The same factory as `flash` (2026-07-27), and since P3b it has **its own column** rather than inheriting flash's: it renders in the **flashPanel** panel with its own `flashSeedling:loadRegion` event and drops the inherited `iframeId`. What the matrix cannot show is the host-side glue that turns the game's own level changes into region moves — see [Flash Substrate](./flash.md#flash_seedling--a-real-games-map-as-procgen-regions). Its content-source hooks (T1) are read off `compileRegionAtlas` of the installed atlas, so its `zoneCount` is the PLACEABLE rooms (an AP region whose compiled payload lists ≥1 wired door), not the atlas's AP-region count. |
| `flash_seedling_gen` | The second Seedling entry (SEEDLING GENERATED LEVELS G1, 2026-09-23): the same panel and load event as `flash_seedling`, but its regions are rooms the Seedling GENERATOR builds to the region's spec, and the sidecar carries the room's record. Its matrix column is the LIGHT entry: the five build hooks delegate to `seedlingDemo/seedlingGenRoom.js`, installed headless by `flashSeedlingGenBuild.js` (the file `REGISTRY_LIBRARIES` names) and in the live app through a computed specifier — see [Flash Substrate § *Generated rooms*](./flash.md#generated-rooms-flash_seedling_gen). |
...
1. **Write the entry** in a `<module>Library.js`: identity, `panelComponentType`, `loadRegionEvent`, `supportedFeatures`, `deserializeWorld`/`serializeWorld` (remember the exits-`Map` requirement), `sidecarFields` (what `serializeWorld` writes — `check-sidecar-fields.mjs` fails every committed entry of a substrate that declares none), `apLocationNamesOf`/`apExitNamesOf` where the payload carries its AP names (optional — absent, the hub's validity report says it did not check them), and whichever optional groups apply. Freeze it and self-register behind a `has()` guard so headless imports work.
...
5. If the substrate participates in generation, implement the build-time group that fits: procedural hooks for grown geometry, or the content-source group (`zoneCount` + `extractZoneRules`) for a fixed ordered pool. A zone whose host reads an exit's `side` and never its tile declares `regionGeometry: REGION_GEOMETRY.SIDES` (§ *Build-time — region geometry*), so the engine mints it no fictional tiles. Declare it on the entry itself, never as a factory default: `createFlashSubstrateEntry` builds every flash-family entry and stays tiles. (This sentence said `flash_seedling` "stays tiles" until T1 gave that entry a build hook; it now declares `'sides'` on its own entry — a placed room's exits are its sides, and the door each is bound to stays where the level put it.) If the payload keys anything by an exit's side (a portal map, a back-exit side, an arrow), declare `exitSides` on the entry too (§ *Editing*) — without it the APWorld editor and the Procgen Pipeline cannot move an exit to another side, and say so. If nothing but the exit's own `side` is keyed by it, declare `SIDE_AGNOSTIC_EXIT_SIDES` — and a side of your regions may then hold more than one exit. If a region costs seconds to generate (generate-and-test), declare `generationCost: 'heavy'` (§ *Build-time — generation cost*), or the first shipped preset that names the substrate puts that cost into the CI slow battery.
