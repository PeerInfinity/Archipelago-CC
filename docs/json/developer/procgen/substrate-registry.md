...
Same-id coexistence is first-wins: `textAdventureSubstrate` (deprecated 2026-07-26) and `textAdventureSubstrateWrapper` both define id `text_adventure`; whichever module loads first owns the id and the other no-ops via the `has()` guard. The deprecated one loads first, so it wins wherever both are enabled — and it declares a strictly smaller `loopSupport` (no `record`/`playback`/`instant`). Every module config now disables it, so the wrapper wins everywhere; that delta is pinned by `textAdventureSubstrateWrapperLibrary.test.js`. See [Gotchas](./gotchas.md#two-text-adventure-modules-register-the-same-substrate-id).
