...
## Generation is NOT deterministic under load — the same seed can produce different levels

Byte-identity above is about the *pipeline*; this is about the *generator*, and it fails the other way. `procgenOracle.js:503` reclassifies a solve that **SUCCEEDED** as `BUDGET_EXHAUSTED` when it took longer than `budget.wallClockMs` (5000) — so elapsed time, which is not a property of the candidate, decides whether that candidate is kept or reverted, and the run reaches different candidates from there on. **Measured 2026-08-13: seed 20, five runs, identical code — three produced 3 levels, two produced `GenerationAborted(PhysicsV2Error)`.** It is not confined to the generator: any test that runs the solver inherits it, and a full vitest suite has gone red with 3 failures at load 22.8 that were 515/515 solo. ⇒ **check `cat /proc/loadavg` before measuring, re-run a seed several times before attributing any difference to a change, and prefer structural claims (a count of shared tags, a chunk count) over timing-sensitive ones.** Note the two `BUDGET_EXHAUSTED` sites differ: the *thrown* path at `:452` classifies `per-target-ticks` deterministically, and only the post-hoc wall-clock branch at `:503` is the defect. The fix — a tick/expansion budget replacing the wall clock — is queued with its measurements and its one real obstacle in [`CC/docs/plans/procgen-deterministic-budget.md`](../../../../CC/docs/plans/procgen-deterministic-budget.md).
