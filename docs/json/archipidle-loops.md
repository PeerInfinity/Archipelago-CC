# ArchipIDLE Loops - User Guide

ArchipIDLE Loops is an incremental game mode built into the ArchipIDLE web client. Inspired by games like *Idle Loops*, *Increlution*, and *Stuck In Time*, it transforms playing through an Archipelago world into an automated, loop-based progression system.
...
-   **Explore Region:**
    *   Gradually reveals *undiscovered* Item Locations and Exits within the current Region.
...
-   **Check Location:**
    *   Attempts to access a *discovered* Item Location.
...
-   **Move to Region:**
    *   Uses a *discovered* Exit to travel to an adjacent Region.
...
-   Gaining levels in a Region grants a **Mana Cost Discount** for all actions performed *in that Region*.
...
-   Separate from Archipelago's concept of accessibility (rule checking). Tracks whether you have *encountered* a Region, Location, or Exit within Loop Mode.
...
-   Retains the standard ArchipIDLE layout (Inventory Left, Console Center, Views Right).
...
-   **Scrollable Area:**
    ...
    *   Regions only appear here once they are discovered *and* an action is queued within them or a move action targets them.
-   **Region Blocks:**
    ...
    *   **Queued Actions Container:** Lists actions queued specifically for *this instance* of the region block in the queue. Each shows:
        *   Action Name.
...
-   **Locations / Exits / Regions Tabs:** These views still function but primarily reflect the *discovered* state. Elements corresponding to undiscovered regions/locations/exits might be hidden or shown as "???". Clicking elements generally has no effect; interaction happens via the Loop Panel.
...
-   **Expand/Collapse All:** Expands or collapses all *currently visible* Region Blocks in the Loop Panel.
-   **(Bottom Bar):** Clear Queue, Save/Load Game, Export/Import State, Hard Reset.

## Getting Started Gameplay Flow

1.  Load your `rules.json` file into ArchipIDLE or use the preset data.
