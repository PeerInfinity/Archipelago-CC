...
class ExtractedData:
    ...
    advancement_locations: Set[str] = field(default_factory=set)  # Locations that should have advancement items (from original placements)
...
def extract_locations(json_data: Dict[str, Any], player_id: str = '1') -> Tuple[Dict[str, LocationData], Dict[str, str], Dict[str, str], Set[str]]:
    """
    ...
    Returns:
        Tuple of (locations dict, original_placements dict, locked_placements dict, advancement_locations set)
    """
    locations: Dict[str, LocationData] = {}
    original_placements: Dict[str, str] = {}
    locked_placements: Dict[str, str] = {}
    advancement_locations: Set[str] = set()
    ...
    for region_name, region_info in regions_data.items():
        for loc_info in region_info.get('locations', []):
            ...
            if item_info:
                ...
                # Track locations that had advancement items (for cross-validation)
                if item_info.get('advancement', False):
                    advancement_locations.add(loc_name)
    ...
    return locations, original_placements, locked_placements, advancement_locations
...
def extract_all(json_data: Dict[str, Any], player_id: str = '1') -> ExtractedData:
    ...
    locations, original_placements, locked_placements, advancement_locations = extract_locations(json_data, player_id=player_id)
    ...
    return ExtractedData(
        ...
        advancement_locations=advancement_locations,
