...
class RuleCodeGenerator:
    ...
    def _convert_ast_function_call(self, rule: Dict[str, Any]) -> str:
        ...
        if option_name:
            ...
            # Known option names that default to False (closed/disabled)
            # These are options that gate progression and should be off by default
            restrictive_options = {'open_pyramid'}
            if option_name in restrictive_options:
                return self._make_bool_constant(False)
