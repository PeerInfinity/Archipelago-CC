...
def generate_rules_py(data: ExtractedData) -> str:
    ...
    # Check if we need SOE-specific helper
    has_soe_helper = bool(data.soe_provides_data.get('item_provides')) or bool(data.soe_provides_data.get('logic_rules'))
    ...
    # Add SOE-specific helper if needed
    if has_soe_helper:
        soe_helper = _generate_soe_helper(data.soe_provides_data)
        if helpers_section:
            helpers_section += '\n' + soe_helper
        else:
            helpers_section = '\n\n' + soe_helper
    ...
    # Add CollectionState import if we have helpers, lambda rules, or SOE helper
    collection_state_import = ''
    if has_helpers or needs_lambda or has_soe_helper:
    ...
    # Build typing import (Dict needed for SOE helper data)
    typing_import = 'from typing import TYPE_CHECKING'
    if has_soe_helper:
        typing_import = 'from typing import Dict, TYPE_CHECKING'
...
def _generate_soe_helper(soe_data: Dict[str, Any]) -> str:
    """Generate SOE-specific helper functions for checking progress requirements.

    This generates:
    - Static data for item_provides and logic_rules
    - _soe_count function that counts progress from items and rules
    - _soe_has function that checks if player has enough progress

    The logic mirrors soeLogic.js in the frontend.
    """
    item_provides = soe_data.get('item_provides', {})
    logic_rules = soe_data.get('logic_rules', [])

    # Generate item_provides dict
    item_provides_entries = []
    for item_name, provides in item_provides.items():
        name_escaped = item_name.replace('\\', '\\\\').replace('"', '\\"')
        provides_list = [f'{{"progress_id": {p.get("progress_id")}, "count": {p.get("count")}}}' for p in provides]
        item_provides_entries.append(f'    "{name_escaped}": [{", ".join(provides_list)}],')

    item_provides_content = '\n'.join(item_provides_entries)

    # Generate logic_rules list
    logic_rules_entries = []
    for rule in logic_rules:
        requires = rule.get('requires', [])
        provides = rule.get('provides', [])
        req_list = [f'{{"count": {r.get("count")}, "progress_id": {r.get("progress_id")}}}' for r in requires]
        prov_list = [f'{{"count": {p.get("count")}, "progress_id": {p.get("progress_id")}}}' for p in provides]
        logic_rules_entries.append(f'    {{"requires": [{", ".join(req_list)}], "provides": [{", ".join(prov_list)}]}},')

    logic_rules_content = '\n'.join(logic_rules_entries)

    return f'''# SOE-specific progress data (from pyevermizer)
