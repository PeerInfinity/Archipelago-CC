...
'''
    ...
    # Generate CANONICAL_PLACEMENTS dict if placement_lookup is used
    canonical_placements_section = ''
    if helper_generator.uses_placement_lookup:
        # Build dict of location -> (item_name, player) tuples
        placement_entries = []
        for loc_name, item_name in sorted(helper_generator.placements.items()):
            loc_escaped = loc_name.replace('\\', '\\\\').replace('"', '\\"')
            item_escaped = item_name.replace('\\', '\\\\').replace('"', '\\"')
            # Format as (item_name, player) tuple - player is always 1 for single-world
            placement_entries.append(f'    "{loc_escaped}": ("{item_escaped}", 1),')
        placements_content = '\n'.join(placement_entries)
        canonical_placements_section = f'''
# Canonical placements from seed generation - used by placement_lookup rules
CANONICAL_PLACEMENTS: Dict[str, tuple] = {{
{placements_content}
}}
'''
    ...
    # Determine if we need Dict import for CANONICAL_PLACEMENTS
    typing_imports = ['TYPE_CHECKING']
    if helper_generator.uses_placement_lookup:
        typing_imports.append('Dict')
    typing_import_str = ', '.join(typing_imports)
...
{canonical_placements_section}{helpers_section}
