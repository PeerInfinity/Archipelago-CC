...
{entrances_content}
    ...
    # However, dynamically_added regions must be included so they get exported
    # with dynamically_added=true instead of being created as placeholder regions.
    for region in regions.values():
        if len(region.locations) > 0 or len(region.exits) > 0 or getattr(region, 'dynamically_added', False):
