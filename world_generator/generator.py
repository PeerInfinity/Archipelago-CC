...
class WorldGenerator:
    ...
    def generate(self, dry_run: bool = False) -> None:
        ...
        # Export settings and helpers for game-specific export handlers
        # This allows worldgen exports to reproduce the same settings/helpers as the source
        with open(self.json_path, 'r') as f:
        ...
        # Export original helper definitions to preserve setting_value references
        # Without this, helper code with setting comparisons gets evaluated at generation
        # time and loses dynamic behavior (e.g., is_not_bunny's mode check becomes constant)
        source_helpers = source_json.get('helpers', {}).get('1', {})
        if source_helpers:
            helpers_path = output_dir / '_worldgen_helpers.json'
            if not dry_run:
                helpers_path.write_text(json.dumps(source_helpers, indent=2))
                logger.info(f"Wrote helpers to {helpers_path}")
