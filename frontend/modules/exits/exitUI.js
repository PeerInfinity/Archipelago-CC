...
export class ExitUI {
  ...
  handleExitClick(exit) {
    ...
    if (this.isDiscoveryModeActive && this.discoverySettings.clickDiscoversLocation) {
      const isExitDiscovered = discoveryStateSingleton.isExitDiscovered(exit.region || exit.parentRegion, exit.name);
      if (!isExitDiscovered) {
        const regionName = exit.region || exit.parentRegion;
        log('info', `[ExitUI] Discovering exit via click: ${exit.name} in ${regionName}`);
