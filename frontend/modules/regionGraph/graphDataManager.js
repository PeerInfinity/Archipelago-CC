...
export class GraphDataManager {
  ...
  showAllLocationNodes() {
    ...
    logger.debug(`Location nodes displayed: ${elementsToAdd.length / 2} locations from ${selectedRegions.length} regions`);
  }
    ...
    for (const regionInfo of candidateRegions) {
      ...
      if (wouldExceedLimit) {
        if (keepRegionSetsComplete) {
          // If we've already added some regions and this would exceed, include it anyway
          // to keep the set complete (going slightly over limit is OK)
          if (totalLocations > 0) {
            selectedRegions.push(regionInfo);
            totalLocations += regionInfo.locationCount;
          }
          // Stop adding more regions after this
          break;
        } else {
          // Strict limit: stop here
          break;
        }
      } else {
        selectedRegions.push(regionInfo);
