...
export const DEMOS = Object.freeze([
    ...
    Object.freeze({
        ...
        demonstrates: 'R9 slice 0: `?width=` `?height=` `?fill=` `?areas=`\n`?require=` and `?elements=` stopped being address-bar-only and got page\ncontrols. ⛔ The claim is the one a control could most easily break: on a URL\nwith NO `?elements=`, the page must still ask for NOTHING — `elementsAsked` is\n`null` — so that the seam applies the BIOME DEFAULT. A control that spelled the\ndefault as `none` would read `{"name":"none"}` here and turn the default off on\nevery load.',
        howToRun: 'Open it and look at the `element` select: it reads\n`(biome default)` and its params box is empty. The readout beside the canvas\nnames what the RUN resolved (`guard+killgate+blockpocket+chamber;w=2;h=3` on\npost-sword) — two different fields, because *what was asked* and *what ran* are\ntwo different facts. Now pick `none` and press RUN-ALL: the bar gains\n`?elements=none`, the ladder RESETS to the skeleton and says which control did\nit, and the room is a different one.',
