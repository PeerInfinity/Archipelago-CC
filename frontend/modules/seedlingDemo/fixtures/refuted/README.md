...
committed gate has to be able to read: that record lives in an untracked
directory, so a test that reached into it would be a gate that disappears on a fresh clone.
