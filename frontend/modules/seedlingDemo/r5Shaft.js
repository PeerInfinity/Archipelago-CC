...
export const TOTEM_PART_2 = Object.freeze({
    ...
    /**
     * ⛓⛓ R5 SLICE 13 — THE APPROACH STANCE, MEASURED.
     *
     * `tile` is where the part IS and a planner may not route onto a pickup
     * (§14's rule: A* refuses a goal tile a pickup occupies), so a collect
     * leg needs a stance BESIDE it. The part's rect is [72,88) x [40,56),
     * which covers four cells; the pocket holding it opens only upward, and
     * (4,1) is wall. ⇒ **(5,1) is the only approach**, and `runCollect`
     * walks the last cell itself.
     *
     * ⛓ AND THE ROUTE TO IT IS THE SHAFT'S OWN PAYOFF: column 9 from the
     * middle of the cross to row 1, which is what `cover t0` at (9,7) —
     * opened by press 18, the one hold no block could otherwise pay for —
     * unseals. The collect is the eighteen presses' receipt.
     */
    stance: Object.freeze({ tx: 5, ty: 1 }),
    /**
     * ⛓⛓ AND THE CEREMONY IS SAFE BY CONSTRUCTION HERE, not by geometry.
     *
     * `crusher.PLAYER_DAMAGE_PATHS`: every damage path in the game reaches
     * the player through `Player.hit`, which is gated on
     * `!Game.freezeObjects`; the one exception is `LavaTrap`, in Dungeons 7
     * and 8. So the 150 frozen frames cannot hurt the player whatever L39's
     * three spinners are doing — and a `Spinner` is a `Mobile`, so
     * `mobileUpdate`'s gate parks it for the duration as well. The stance
     * claim is POSITIONAL, and it is discharged by the walk arriving.
     */
    ceremonySafety: 'positional — Player.hit is freeze-gated and a Spinner is a Mobile',
    /**
     * ⛔ AND THE APPROACH AIMS STRAIGHT DOWN, not at the part's centre.
     *
     * The rect is [72,88) x [40,56) — it straddles the column boundary at
     * x = 80 — and the only free cell above the pocket is (5,1), whose own
     * west neighbour (4,1) is `tile:Blue Wall (dark)`. A walk from (88,24)
     * toward the centre (80,48) drifts west and the sweep is refused at
     * x = 82.76, four pixels before it touches anything.
     *
     * ⛓ Straight down from (88,24) enters the volume at y = 40: the player
     * box is [86,90) and the rect's right edge is 88, so the overlap is two
     * pixels wide and real. `runCollect` still requires actual contact — the
     * override moves the AIM, not the test.
     */
    aim: Object.freeze({
        x: 88,
        y: 48,
        why: 'the part\'s rect straddles x = 80 and (4,1) is wall, so the line from the '
            + 'only approach cell to its centre drifts into the wall. Straight down from '
            + '(5,1) makes contact on the rect\'s east two pixels.',
    }),
...
export const SHAFT_PAIR = Object.freeze({
    ...
    /**
     * ⛔⛔ R5 SLICE 13 — THE PIN WAS A PREDICTION, AND DRIVING IT REFUTED IT.
     *
     * §24.9 recorded that this arm *"has never been generated by anything"*,
     * and `pinnedAt: (9,5)` — *"the tile below the lowest WandLock"* — was a
     * hand-derived guess about where a pressless walk would come to rest. It
     * assumed the control walks north until the first blocker stops it.
     *
     * ⛓ THAT IS NOT WHAT A CONTROL IS. It replays the press arm's whole
     * INPUT tape into a shut world, so after it is first blocked the
     * remaining spans keep pushing it around; where it stops is an artefact
     * of the last span, not a statement about the room. Driven, it stops at
     * (11,15) — and the fact worth banking is a different, stronger one:
     *
     * ⛓⛓ **IT NEVER REACHES ROW 9 AT ALL.** The three covers hold the whole
     * crossing, so the highest row the pressless walk touches is 11 — where
     * block 1 still stands on its spawn — and (9,9), the middle of the cross
     * the press arm ends in, is not on its path on any tick.
     *
     * ⚠ AND IT MOVED WITHIN THIS SLICE, WHICH IS THE POINT. Adding the
     * collect leg gave the press tape 67 more spans; the control keeps all
     * of them, walks further, and comes to rest at (10,13) instead of
     * (11,15). Nothing about the ROOM changed. A pair whose claim was the
     * resting cell would have had to be re-derived every time the tape grew
     * a tail — which is exactly the kind of expectation that gets tuned
     * until green rather than re-measured.
     *
     * The resting cell is kept as REPORTED DATA (it catches drift); the
     * claim is `highestRowReached` and `neverEnters`.
     */
    pinnedAt: Object.freeze({ level: LEVEL, tile: Object.freeze({ tx: 10, ty: 13 }) }),
    predictedPin: Object.freeze({ tile: Object.freeze({ tx: 9, ty: 5 }), refuted: true }),
    /** ⛓⛓ The load-bearing negative: the crossing is shut, measured. */
    neverEnters: Object.freeze({ tx: 9, ty: 9 }),
    highestRowReached: 11,
    /**
     * ⚠ THE CONTROL PULLS THE ROPE TOO, and it has to: the rope's shrink is
     * what opens the shaft, and a control stopped at the door would be
     * testing the door. So its ledger is the rope's own writes plus the
     * declared entrance flag.
     *
     * ⛔ AND THE BANKED LIST WAS SHORT BY THE ROCK. Slice 7 wrote this before
     * slice 10 found that `RopeStart`'s publication also calls
     * `FallRock.fall()` — {39,10}, at pull time. A control that pulls the
     * rope drops the rock too.
     */
    controlLedger: Object.freeze(['39:4', '39:8', '39:9', '39:10']),
    /**
     * ⛔⛔⛔ AND THE CONTROL EARNS ONE FLAG NOBODY PLANNED: {39,4}.
     *
     * `spinner@224,112`'s own tag. Nothing on this arm fights anything — the
     * eighteen presses are DELETED — and the game's ledger carried it
     * anyway. ⛓ `Pulser.hit`'s third arm is
     * `(c as Enemy).hit(force, …, damage, "Pulse")`, and with the blocks
     * left at their spawns the billiard's path is different enough to cross
     * the pulser's 22 px ring three times. The PRESS arm's spinner never
     * does: three moved blocks are three moved walls.
     *
     * ⛓⛓ **THE CONTROL IS THEREFORE A REAL EXPERIMENT AND NOT A FORMALITY.**
     * Deleting eighteen presses changed which enemy lived — a second-order
     * consequence nothing about the pair predicted, that the model now
     * reproduces exactly (`run.spinnerWrites`), and that would have read as
     * an unexplained ledger disagreement for a whole slice if the spinner
     * had stayed unmodelled.
     */
    controlEarned: Object.freeze(['39:4', '39:9', '39:10']),
    controlSpinnerKill: Object.freeze({
        id: 'spinner@224,112', tag: 4, by: 'Pulser', arm: 'Pulser.as:110',
        why: 'the blocks stay on their spawns in this arm, so the billiard takes a '
            + 'different path and crosses the ring',
    }),
