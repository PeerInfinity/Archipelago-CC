...
/**
 * ⛓ THE BROWSER-SAFE THREE, imported and re-exported (editor arc slice 1).
 *
 * `R6AcceptanceError`, `FP_ELAPSED_CLAMPED` and `animCallbackUpdate` are
 * defined in `r6AnimClock.js` because THIS module imports `node:fs`
 * transitively and `endingChain` — a module the live run needs — imports
 * the animation transcription. See that file for the whole reason.
 *
 * ⚠ IMPORTED, then re-exported from the LOCAL bindings: this module throws
 * `R6AcceptanceError` in many places and a bare `export … from` would
 * create no local binding for them.
 */
import { FP_ELAPSED_CLAMPED, R6AcceptanceError, animCallbackUpdate } from './r6AnimClock.js';

export { FP_ELAPSED_CLAMPED, R6AcceptanceError, animCallbackUpdate };
