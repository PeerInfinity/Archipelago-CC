/**
 * seedlingDemo/watchLifetime — WHO OWNS THE PAGE RIGHT NOW, and what has to
...
export class WatchLifetimeError extends Error {
    constructor(message) {
        super(message);
        this.name = 'WatchLifetimeError';
    }
}

const fail = (message) => { throw new WatchLifetimeError(message); };
...
export function createLifetime(name, generation = 0) {
    if (typeof name !== 'string' || !name) {
        fail('watchLifetime: a lifetime needs a NAME — it is what the readout and every '
    ...
    return {
         ...
         * that is supposed to catch it. `watchLifetime.test.js` asserts the
        ...
        on(target, type, handler, options = undefined) {
            if (!alive()) {
                fail(`watchLifetime: ${name}#${generation} is retired (${retiredWhy}) and `
                    + `cannot listen for "${type}". An aborted signal would make this a `
                    + 'silent no-op, so it is a refusal instead.');
            }
            if (!target || typeof target.addEventListener !== 'function') {
                fail(`watchLifetime: ${name}#${generation} was given a "${type}" target that `
        ...
        guard(loopName, fn) {
            if (typeof loopName !== 'string' || !loopName) {
                fail('watchLifetime: a guarded loop needs a NAME — the readout lists what it '
        ...
        onRetire(fn) {
            if (typeof fn !== 'function') fail('watchLifetime: onRetire needs a function.');
            if (!alive()) {
...
export function createLifetimeHolder({ keep = 8, publish = null } = {}) {
     ...
     * retirement and `watchLifetime.test.js`'s "keeps the retired arms" row
