...
export async function shipToWasm(payload, host) {
    ...
    for (let k = 0; k < windows.length; k += 1) {
        ...
        /**
         * ⛓ NO `rec.arm` SINCE SLICE R2 (2026-09-12). R9 12g′ surfaced
         * `botStatus.arm` here so a consumer asserting `deadFrames` could tell a
         * build that arms after the world swap (p4c and later) from one that
         * armed beside it (p4b). Its one reader, `check-seedling-wasm-ship.mjs`'s
         * CLAIM 6, folded to the arm-after-swap value when p4b retired (⚖ user:
