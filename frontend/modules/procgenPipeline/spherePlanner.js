...
export function planSpheres({
    ...
    //    unpinned remainder, drawing in shuffled order.
    ...
    // 3. Even distribution of the remainder: fill each sphere toward a
    //    balanced target (earlier spheres take the modulo extras), then
    //    drain anything left — overflow from pin-heavy spheres — onto
    //    the currently-smallest sphere.
    const base = Math.floor(total / n);
    const extras = total % n;
    for (let k = 0; k < n && remaining.length > 0; k++) {
        const target = base + (k < extras ? 1 : 0);
