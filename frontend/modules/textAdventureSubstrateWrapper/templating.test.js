 ...
 * textAdventureSubstrateTemplating.test.js (2026-07-26). The wrapper carried a
 * mirrored templating module with NO tests of its own, so this is the old
 * module's coverage following the code that survived it.
 *
 * Two deliberate differences from the original, both real behaviour changes in
 * the wrapper rather than transcription drift:
 *
 *  - the item highlight span is `tae-item-name` (the engine's own class), not
 *    the old panel's `item-name`;
 *  - substituted values are HTML-ESCAPED. The old module interpolated
 *    `String(value)` raw, so custom-data prose could inject markup into the
 *    panel. That escaping had no coverage at all, which is why the escaping
 *    cases below are new rather than ported.
 */
...
describe('processMessageTemplate', () => {
    ...
    it('wraps {item} in the engine item-name span when wasUnchecked is true', () => {
        const out = processMessageTemplate('found {item}', { item: 'sword', wasUnchecked: true });
        expect(out).toBe('found <span class="tae-item-name">sword</span>');
    });
    ...
    it('never emits wasUnchecked itself as a substitution', () => {
        expect(processMessageTemplate('a{wasUnchecked}b', { wasUnchecked: true }))
            .toBe('a{wasUnchecked}b');
    });

    // Escaping: wrapper-only behaviour, previously untested. Custom data is a
    // fetched JSON document, so its prose must not be able to inject markup
    // into the engine's message log.
    it('escapes HTML in substituted values', () => {
        expect(processMessageTemplate('{v}', { v: '<b>x</b>' }))
            .toBe('&lt;b&gt;x&lt;/b&gt;');
    });

    it('escapes quotes and ampersands', () => {
        expect(processMessageTemplate('{v}', { v: `&"'` }))
            .toBe('&amp;&quot;&#39;');
    });

    it('escapes the item value INSIDE the highlight span', () => {
        const out = processMessageTemplate('{item}', { item: '<img>', wasUnchecked: true });
        expect(out).toBe('<span class="tae-item-name">&lt;img&gt;</span>');
    });

    it('does not escape the template itself, only the values', () => {
        expect(processMessageTemplate('<i>{v}</i>', { v: 'x' })).toBe('<i>x</i>');
    });
});
...
describe('custom*Message lookup helpers', () => {
    ...
    it('customLocationCheckMessage passes through item + wasUnchecked', () => {
        ...
        expect(out).toBe('You search Slay Yorgle and find <span class="tae-item-name">sword</span>.');
    });
    ...
    it('caller vars override the helper-supplied name', () => {
        expect(customRegionEnterMessage(data, 'Overworld', { regionName: 'Elsewhere' }))
            .toBe('Welcome to Elsewhere.');
    });
    ...
    it('all lookups tolerate null / undefined custom data', () => {
        for (const d of [null, undefined]) {
            expect(customRegionEnterMessage(d, 'X')).toBeNull();
