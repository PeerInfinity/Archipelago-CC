      ...
      if (sm.autoCollectEventsEnabled) {
        for (const loc of sm.eventLocations.values()) {
          if (sm.knownReachableRegions.has(loc.region)) {
            ...
            // Note: We don't check _hasItem because the same item can come from multiple locations
            // (e.g., KDL3 has 6 "Stage Completion" items from 6 different stage locations)
