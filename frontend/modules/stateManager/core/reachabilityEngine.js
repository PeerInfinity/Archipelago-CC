      ...
      if (sm.autoCollectEventsEnabled) {
        for (const loc of sm.eventLocations.values()) {
          if (sm.knownReachableRegions.has(loc.region)) {
            ...
            // Note: We don't check _hasItem because multiple events can give the same item
            // (e.g., multiple Star events in Overcooked! 2). checkedLocations prevents duplicates.
