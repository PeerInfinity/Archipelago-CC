...
/**
 * Load the sphere log EMBEDDED on an in-memory rules doc (procgen pipeline
 * output, editor Apply, or localStorage-restored rules). These sources have no
 * preset file path to fetch, but buildRulesJson embeds `sphere_log` directly.
 * Pulls the last raw JSON stateManager received and loads its embedded entries
 * (the same array→jsonl shape loadEmbeddedFirstThenFile uses for file presets).
 * Returns true when a non-empty embedded log was found and a load kicked off.
 */
function tryLoadEmbeddedSphereLog(sphereState, sourceName) {
  const entries = getLastRawJsonData?.()?.rawJsonData?.sphere_log;
  if (!Array.isArray(entries) || entries.length === 0) {
    log('info', `No embedded sphere_log on the loaded rules (${sourceName})`);
    return false;
  }
  const jsonlText = entries.map((e) => JSON.stringify(e)).join('\n');
  sphereState.loadSphereLog(`${sourceName}:embedded`, jsonlText).then((success) => {
    log(success ? 'info' : 'warn',
      success
        ? `Loaded ${entries.length} embedded sphere entries (${sourceName})`
        : `Failed to load embedded sphere log (${sourceName})`);
  });
  return true;
}
...
function handleRulesLoaded(data, propagationOptions) {
  ...
  if (!match) {
    ...
    // In-memory sources (procgen pipeline, editor Apply, localStorage-restored
    // rules) have no preset file path to fetch, but their rules doc may carry an
    // embedded sphere_log (buildRulesJson embeds it by default). Load that
    // directly from the last raw JSON stateManager received — otherwise the
    // playback bot / cost debugger sit on "No sphere log loaded" even though the
    // generated world ships its own log.
    if (isFromProcgen || isFromEditor || isFromLocalStorage) {
      tryLoadEmbeddedSphereLog(sphereState, sourceName);
    }
