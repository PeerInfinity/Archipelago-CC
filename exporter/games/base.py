...
class BaseGameExportHandler:
    ...
    def get_helper_definitions(self, world) -> Dict[str, Any]:
        ...
        """
        # Check for worldgen worlds first - use pre-defined helper definitions from Rules.py
        try:
            world_module = type(world).__module__
            if '_worldgen' in world_module:
                # This is a worldgen world - get pre-defined helper definitions from Rules.py
                ...
                try:
                    ...
                    # First, try to use pre-defined helper definitions from get_helper_definitions()
                    # These are generated by the world generator and contain the complete
                    # helper definitions including option-dependent conditionals that would be
                    # lost when re-analyzing the resolved Python code
                    if hasattr(rules_module, 'get_helper_definitions'):
                        predefined_helpers = rules_module.get_helper_definitions()
                        if predefined_helpers:
                            logger.debug(f"Using {len(predefined_helpers)} pre-defined helper definitions from worldgen Rules.py")
                            return predefined_helpers

                    # Fall back to analyzing helper functions if no pre-defined definitions
