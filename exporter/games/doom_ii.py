"""DOOM II game-specific rule expansion handler."""
...
class Doom2GameExportHandler(BaseGameExportHandler):
    ...
    def expand_helper(self, helper_name: str):
        """Expand DOOM II specific helper functions."""
        ...
        if expansion:
            logger.debug(f"Expanded DOOM II helper {helper_name} to {expansion}")
            return expansion
        
        logger.debug(f"Unknown DOOM II helper: {helper_name}")
        return None  # Let base class handle or preserve as-is
    
    def expand_rule(self, rule: Dict[str, Any]) -> Dict[str, Any]:
        """Recursively expand DOOM II rules."""
        if not rule:
            return rule
    ...
    def get_item_data(self, world) -> Dict[str, Dict[str, Any]]:
        """Get DOOM II specific item data."""
