...
class WorldDataMixin:
    ...
    def get_world_data(self, world, multiworld, player) -> Dict[str, Any]:
        ...
        # For worldgen worlds (generated by world_generator), enable counting of non-advancement
        # items by default. Worldgen worlds use Rule Builder rules with Has() which need to count
        # ALL items (not just advancement) to match Python's state.has() behavior.
        # This is critical for games like Landstalker where items like EkeEke are needed for
        # entrance rules but most copies are marked as non-advancement (filler).
