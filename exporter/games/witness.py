...
class WitnessGameExportHandler(GenericGameExportHandler):
    ...
    def _simplify_all_of_with_mixed_conditions(self, rule: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        # Convert bound methods to reach_region rule types
        # We use a dedicated rule type instead of a helper to avoid the generic handler
        # expanding it into an uninformative capability rule
        for bm in bound_methods:
            if hasattr(bm, '__self__') and hasattr(bm.__self__, 'name'):
                ...
                logger.debug(f"Converting bound method to reach_region rule for '{region_name}'")
                ...
                try:
                    ...
                    logger.debug(f"Converting bound method string to reach_region rule for '{region_name}'")
                    analyzed_conditions.append({
                        'type': 'reach_region',
                        'region': region_name
                    })
    ...
    def _simplify_region_reachability_pattern(self, rule: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        if self._is_region_reachability_pattern(rule):
            ...
            if hasattr(self, '_exit_region_names') and self._exit_region_names:
                ...
                logger.debug(f"Converting region reachability pattern to reach_region('{region_name}')")
                return {'type': 'reach_region', 'region': region_name}
