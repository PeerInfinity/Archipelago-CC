"""
Serializer for Stardew Valley's custom StardewRule objects.

Stardew Valley uses a custom rule system instead of lambda functions.
This module detects and serializes those rule objects to JSON format.
"""
...
def is_stardew_rule(obj: Any) -> bool:
    """
...
def serialize_stardew_rule(rule_obj: Any, player_context: Optional[int] = None) -> Optional[Dict[str, Any]]:
    """
    Serialize a StardewRule object to JSON format.

    Args:
        rule_obj: The StardewRule object to serialize
        player_context: The player number for context
    ...
    try:
        ...
        # Handle And rule (logical AND)
            ...
            if hasattr(rule_obj, 'original_rules'):
                for sub_rule in rule_obj.original_rules:
                    serialized = serialize_stardew_rule(sub_rule, player_context)
                    if serialized:
        ...
        # Handle Has rule (wrapper that delegates to underlying item rule)
            ...
            if hasattr(rule_obj, 'other_rules') and item_name in rule_obj.other_rules:
                ...
                return serialize_stardew_rule(underlying_rule, player_context)
            else:
