...
def build_prompt(game_name, game_info, seed=1):
    ...
    prompt = f"""Please read CC/exporter-debugging.md.
...
Before you begin working, read remaining-exporter-issues.md

Work on just one issue at a time, or one set of related issues.

After choosing an issue to work on, run the generation script to confirm that the issue still exists, and then begin working on fixing it.

The exporter will output the rules.json file to frontend/presets/{world_dir}/{seed_id}/{seed_id}_rules.json

This is the file that you will need to check, to see if the data has been successfully exported.
