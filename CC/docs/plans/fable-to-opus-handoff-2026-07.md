...
## 5p. REVIEW R1 — the region marking tool ↔ the editor — DONE 2026-09-06 (Fable session `next-priorities-planning-3` at main `ed4899e5f5`; successor to §5o's HANDOFF; memory `project_apworld_editor_hub`)

**The ask (user, 2026-09-05/06):** *"The region marking tool is still experimental. We should review what it does
and how it connects to the other components before we decide to enable it."* / *"I'm not aware of anything that
depends on it."* Everything below was MEASURED at `ed4899e5f5`; the door drive is a scratch Playwright script
(`door-drive.mjs`, scratchpad, not kept) run against an UNCOMMITTED scratch mode (`modes.json` entry `rmt-review`
→ `modules-rmt-review.json` = the default module set + `regionMarkingTool` on; reverted after the conversation).

**What it is.** `frontend/modules/regionMarkingTool/`: 9 files / 3,603 lines (model `atlasSession` +
`atlasEditAdapter` + `mapSource` 784; panel `regionMarkingToolUI` + `markingRenderer` + css + `index` 1,641;
tests 1,178). Bounded vitest **79/79**; `check-region-marking-tool.mjs` under `?mode=flash` **50 PASS / 0 FAIL**;
**0 page errors** on the flash, default and scratch pages; the flash page's console errors are **2**, both the dev
server's benign `/_source-mtime` 404 (already in `check-preset-bundle-load`'s BENIGN_404) — the handoff's "89
console errors" does NOT reproduce. **Seedling-only by construction**: `MAP_DOCUMENT_URL` hardcoded, `ANALYZERS`
= `{seedling}`, `GAME_CONFIG_URL` under `flashPanel/games/`, `DEFAULT_MARKER_TYPES` = Seedling entity names; RWK
postponed indefinitely 2026-07-28. **Neither committed real atlas is authored in the panel**: `seedling.json` is
rebuilt by `make-seedling-starter-atlas.mjs` (README: "derived, not typed"), `seedling-playthrough.json` is
generated by `make-seedling-playthrough-rules.mjs` via `seedlingAtlasDerivation` ("GENERATED — do not edit").
What those scripts and both derivations (`seedlingAtlasDerivation`, `mazeAtlasDerivation`) reuse is the MODEL;
the PANEL's only production caller is the hub's `region_atlas` door (`git grep openRegionMarkingTool`: 1 caller).
`region_atlas` at runtime: exactly ONE reader, `flashPanel/mapDocumentPath.js` (`map_document`); nothing reads
`atlas_id` or `game` — H5's "nothing resolves it" holds for the id only. The default layout ALREADY carries the
panel (`layout_presets.json` default, column 2), dropped silently while the module is off. No module doc, no user
guide, not on the user-docs TODO.

**The round trip, DRIVEN end to end for the first time since L4** (hub → `_openDocumentKeyEditor` → tool → real
Save button → `_acceptEditorOp` with R-a's token and L4's veto on the path), **14/14**: the door is ENABLED with
the module on; the hub's `onSave` lands in the tool; Save hands back exactly ONE document-scope
`set-key region_atlas` (`seedling-ae833c1e` → `seedling-ab0b2709`); the hub prints "applied as one `set-key`";
ONE hub Undo restores value and op list; a Save made for a document the hub since replaced is REFUSED by the
token guard (op list unmoved); the reverse "Open in APWorld Editor" opens a NEW hub session (token moved) holding
the compiled 11-region world. ⚠ **Two findings.** (1) **The hand-off writes a reference to an atlas no file
holds**: with `onSave` installed, `_save` hands the stamped document over and does NOT download it, and the hub
keeps only `{atlas_id, game, map_document}` — an EDITED atlas exists only in the tool's memory and the document
names an id nothing on disk carries (hidden in the drive because the loaded atlas was unedited). (2) **The
handoff's live link loads a document with NO `region_atlas`**: `?game=seedling_playthrough&seed=1` resolves via
`preset_files.json` to the Generate.py folder `AP_14089154938208861744`, which carries no block; the compiled
document is reachable by `?rules=./presets/seedling_playthrough/AP_1/AP_1_rules.json` only (the in-app row loads
it by path, so it never noticed). H6a's parked subject (the maze derivation's payload shape) is atlas-side and
independent of this panel.

**Planner's answers:** (1) an AUTHORING tool for us, Seedling-only — not a product surface; (2) NOT in the default
config (flash mode keeps it; the layout slot is harmless); (3) keep the door, close the phantom reference (Save in
hand-off mode also downloads, one branch in `_save`, or the door's note says the atlas is not persisted); the
atlas RESOLVER (H6a's line) is not worth building while nothing edits atlases in the panel; **H6a stays parked**.

**⚖ RULED (user, 2026-09-06), verbatim:** *"This tool is still experimental, and might already be obsolete. I
would recommend keeping it disabled in the default layout. If there are simple fixes we could apply to make it
fully functional, then we could do that, but it's a low priority."* ⇒ `modules.json` UNCHANGED; **no arc opened;
nothing launched**; H6a stays PARKED; the ⚖ 1 / ⚖ 3 of hub plan §19.6 are CLOSED by this ruling (⚖ 3, the
`MAP_DOCUMENT_URL` fourth spelling, joins the low-priority list). **Low-priority "simple fixes", named, none
launched:** (a) hand-off Save also downloads the stamped atlas (or the door's note says it is not persisted);
(b) route `MAP_DOCUMENT_URL` through `flashPanel/mapDocumentPath` (one line + a mount option); (c) the
...
**⚖ RULED (user, 2026-09-07): "Yes, I authorize the push."** **W1 VERIFIED by the planner 2026-09-07** — re-derived
independently of the block above, on disk and over the wire: `168f9968b2` on origin/main, 3 commits by path (gate
`881076b540`, gitlink `f75b0b6d47` = `e645f2e..2bbabb2`, record); the submodule at `2bbabb2` with 4 tracked build
dirs and `seedling_original` `demo: true` / `capabilities: []` / wasm md5 `9a973e54…`; the live page 200 with the
wasm at 33,633,924 B by `content-length`; `check-seedling-wasm-pins` ALL PASS — 4 pinned builds; `seedling-wasm.yml`
SUCCESS at that SHA; `ci-vitest-summary.mjs 168f9968b2` 442/13384, 0 failed, ±0 as derived (the gate has no vitest
file). Rulings 1, 2, 4, 5, 7 honoured as the block above records; ruling 3's site is serving. ⚑ The two
verification blocks in this section were written 22 s apart by the slice and the planner into ONE tree (no bytes
