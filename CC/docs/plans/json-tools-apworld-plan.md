...
1. **Stub World Requirement**: Does Archipelago require apworlds to define a game with items/locations? Can we create a tool-only apworld?

2. **Frontend Distribution**: Bundle in apworld (~5MB compressed) or separate download?

3. **Settings UI**: Can we add settings to the Options Creator without patching?

4. **Version Compatibility**: How to handle AP version updates that break patches?

5. **World File Patches**: The 11 world `__init__.py` patches for `skip_required_files` - are these needed for basic export functionality?
