...
def call_generate(yaml_path, fuzz_args, output_path, generation_seed=None):
    from settings import get_settings
...
if __name__ == "__main__":
    ...
    def main(p, args, tmp):
        ...
        # [fork] Parse default_options into a set
        if args.default_options:
            default_options = set(opt.strip() for opt in args.default_options.split(","))
        else:
            default_options = set()

        # [fork] Parse disallow_options into a dict mapping option names to sets of disallowed values
        ...
        while i < args.runs and not STOP_REQUESTED:
            # [fork] Seed random for this iteration if starting_seed is provided
            # (deterministic YAML option generation in the main process)
            if args.starting_seed is not None:
            ...
            if args.sample_from:
                ...
                # [fork] thread default_options / disallow_options into YAML generation
                yamls_to_write = [
                    (f"{i}-{nb}.yaml", generate_random_yaml(game, meta, default_options, disallow_options))
