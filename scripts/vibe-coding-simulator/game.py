...
def print_status(state: GameState):
    ...
    # Claw instances
    print("\n  Claw Instances:")
    for inst in state.claw_instances:
        if inst.is_idle:
...
def print_available_actions(state: GameState):
    ...
    idle_instances = [i for i in state.claw_instances if i.is_idle]

    print("\n  Actions:")
    if state.pending_merge_conflicts and idle_instances:
        print(f"    resolve <instance#> <node_id> — assign Claw to resolve merge conflict")
        print(f"    Pending conflicts: {', '.join(state.pending_merge_conflicts)}")

    if idle_instances and available:
        print(f"    assign <instance#> <node_id>  — assign Claw to implement a phase")
        ...
        print(f"    (all Claw instances busy)")
    elif not available:
        print(f"    (no unlocked phases available)")

    busy = [i for i in state.claw_instances if not i.is_idle]
    if busy:
...
def handle_input(state: GameState, line: str) -> bool:
    ...
    if cmd == "quit" or cmd == "q":
        ...
        instance = next((i for i in state.claw_instances if i.id == inst_id), None)
        if not instance:
        ...
        if instance and not instance.is_idle:
            ...
            state.log.append(f"[{state._time_str()}] Claw #{instance.id}: skipped regression testing")
        else:
...
def main():
    ...
    parser.add_argument("--instances", type=int, default=2, help="Number of Claw instances")
    ...
    print(f"  Claw instances: {args.instances}")
    ...
    if state.is_complete():
        ...
        print(f"  Total credits used: {sum(i.credits_used for i in state.claw_instances):.0f}")
