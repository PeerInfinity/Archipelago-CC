...
def is_canonical_difference(path: str, original_value: Any = None, worldgen_value: Any = None) -> bool:
    ...
    # exclude_locations length difference when original uses enable_* options
    # Original may have pre-populated exclude_locations based on enable_* settings
    if 'options.exclude_locations[length]' in path:
        return True
