...
def is_canonical_difference(path: str, original_value: Any = None, worldgen_value: Any = None) -> bool:
    ...
    if 'access_rule' in path:
        ...
        if '[length]' in path:
            return True
        # WrapperRule uses "child" instead of "args.condition" for unary rules like Not
        # Original AST export: {"rule": "Not", "args": {"condition": {...}}}
        # WorldGen rule_builder: {"rule": "Not", "child": {...}}
        if '.child' in path or path.endswith('.child'):
            return True
