...
def load_ut_fuzz_timeout_overrides(project_root: str = None) -> Dict[str, int]:
    """
    Load game-specific timeout overrides for UT fuzz testing.

    These overrides are for games that need longer timeouts due to complex
    option combinations (e.g., large start_inventory values generated by fuzzer).

    Args:
        project_root: Optional project root path. If not provided, will be inferred.

    Returns:
        Dict mapping template filename (e.g., "Shivers.yaml") to timeout in seconds.
    """
    if project_root is None:
        project_root = os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
    ...
    try:
        with open(exclude_list_file, 'r') as f:
            data = json.load(f)
            overrides = data.get('ut_fuzz_timeout_overrides', {})
            # Filter out the comment key
            return {k: v for k, v in overrides.items() if k != 'comment' and isinstance(v, int)}
