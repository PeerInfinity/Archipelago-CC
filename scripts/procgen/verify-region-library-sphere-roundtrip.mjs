 ...
 * SPHERE-GROWTH mode (region-library F6a, the sphere analogue of
 * verify-region-library-roundtrip.mjs). Proves that a sphere world mixing a
 * committed BOUNCE library entry with generated bounce regions survives the whole
 * toolchain and yields a WINNABLE seed:
 *
 *   committed demo pack (frontend/region-libraries/demo-bounce-pack.json)
 ...
 * the emitted logic — it does NOT share the JS placement's assumptions. A
 * library-placed region IS a self-contained bounce region (no library residency
 * in the compiled world), and its relocated locations must survive every hop and
 * be reachable/checked.
 *
 * Runs in a throwaway world/preset (region_library_sphere*) and cleans up.
 * Requires the repo Python env. Run:
 *   node scripts/procgen/verify-region-library-sphere-roundtrip.mjs
 */
...
const WORLD_DIR = path.join(repoRoot, 'worlds/region_library_sphere_worldgen');
const GAME_NAME = 'Region Library Sphere';
const PRESET_DIR = path.join(repoRoot, 'frontend/presets/region_library_sphere_worldgen');
const PRESET_FILES_JSON = path.join(repoRoot, 'frontend/presets/preset_files.json');
const OUTPUT_DIR = path.join(repoRoot, 'output');
const LIBRARY_FILE = path.join(repoRoot, 'frontend/region-libraries/demo-bounce-pack.json');
const SEED = Number(process.env.RL_SPH_SEED || 3);
const tmpRules = path.join(repoRoot, 'scripts/procgen/.rl-sphere-rules.json');
const tmpTemplates = path.join(repoRoot, 'scripts/procgen/.rl-sphere-templates');
...
try {
    ...
    const { GATEABLE_ITEMS } = await imp('frontend/modules/bounceDemo/bounceDemoLibrary.js');
    ...
    // Load + validate the committed demo bounce pack (capability check via bounce).
    const lib = JSON.parse(fs.readFileSync(LIBRARY_FILE, 'utf8'));
    const vr = validator.validateRegionLibrary(lib, { entryCapabilityCheck: (e) => bounce.validateLibraryEntry(e) });
    ok(vr.ok, `demo bounce pack validates (${vr.errors.join('; ') || 'no errors'})`);

    const srcId = engine.librarySourceId(lib.library_id);
    const plan = planSpheres({
        itemPool: { 'Right arrow': 1, 'Left arrow': 1, Springs: 1, Jetpacks: 1, 'Blue platforms': 1, Victory: 1 },
        sphereCount: 3, exclusiveSpheres: { 1: ['Right arrow'] },
        victoryItem: 'Victory', gateableItems: GATEABLE_ITEMS, seed: 1,
    });
    // Mix generated bounce regions with library slots (the start region is a
    // generated bounce region; library entries fill non-root nodes).
    const { grid, startCell, tree } = engine.growSpheres({
        regionSize: { width: 8, height: 6 }, seed: SEED,
        regionParams: { fallBehavior: 'current' },
        growthParams: {
            spherePlan: plan,
            substrateQuotas: { bounce: 3, [srcId]: 6 },
            substrateConfig: { [srcId]: { libraryDoc: lib } },
            startSubstrate: 'bounce', maxItemsPerRegion: 2, assumeBidirectional: true,
        },
    });
    ...
    // Library-placed regions render as bounce (self-contained; no library kind).
    for (const n of tree.nodes.filter((x) => x.substrate === srcId)) {
        ok(grid.getRegion(n.cell).substrate === 'bounce', `Pass A: ${grid.getRegion(n.cell).region_id} renders as bounce`);
    }
...
console.log(failures === 0
    ? '\nAll sphere region-library round-trip assertions passed.'
