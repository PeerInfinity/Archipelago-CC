...
/**
 * ⛓ EVERYTHING THIS FILE DRIVES LIVES IN `main()`, AND `main()` RUNS ONLY WHEN
 * THIS FILE IS THE ENTRY POINT — the IMPORT door of `check-procgen-help.mjs`.
 * Before V2 the box lock, the corpus scan and `chromium.launch()` all ran at
 * module scope, so a bare `await import()` — which is what a vitest file
 * importing anything from this directory does — TOOK THE REAL BOX and drove a
 * headless browser. Everything above this line is a declaration and stays out
 * here, where the derivations remain importable. The pattern is
 * `check-slice-records.mjs`'s.
 */
async function main() {
