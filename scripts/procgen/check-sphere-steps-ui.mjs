 ...
 * varying regenerate lives in check-region-step-editing's Phase F.
 *
 ...
 * Run: node scripts/procgen/check-sphere-steps-ui.mjs
 * @ci-box V3b adopted this script's NAME, not its RUN: it drives a repo-root dev server at a hardcoded `localhost:8000` and it reads `--host=` by hand rather than via `arg('host')`, so the roster cannot point it elsewhere.
 ...
 *
 * ⇒ THE VARYING REGENERATE IS `check-region-step-editing.mjs`'s PHASE F,
...
if (hMoved.length) {
    throw new Error(`H: the contract no longer PINS this region — [${hMoved.join(', ')}] moved `
        ...
        + "check-region-step-editing's Phase F does.");
}
...
// covered by check-region-step-editing's engine cases G/H; here we just
