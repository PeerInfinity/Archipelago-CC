 ...
 * location each, loop_costs embedded. ⛔ The region costs are NOT quoted here
 * any more: they are whatever the ONE cost model writes for this fixture, and
 * they MOVED when that model landed (50/55/60 → 50/32/23, loop-costs L2
 * 2026-09-06). Everything numeric below is derived from the block on disk.
 *
 * What it asserts:
 ...
 * Run: node scripts/procgen/check-maze-loop-mana.mjs
 * @ci-box V3b adopted this script's NAME, not its RUN: its fixture `maze_loop_worldgen` is regenerated by hand and NOT committed, so a checkout has nothing for it to drive.
...
/**
 * ⛓⛓ **THE PER-TILE CAP IS DERIVED FROM THE FIXTURE'S OWN BLOCK, never typed.**
 * A DELEGATED maze charge is the region's `moveCost` ÷ its
 * `longestShortestPath` (`mazeRoomUI._perTileMoveCost`); the coarse path
 * charges the whole `moveCost` once, per action. So the largest per-tile charge
 * this block can produce is the highest threshold that still admits every real
 * per-tile drop, and anything above it is coarse by construction.
 *
 * ⚠ **IT CANNOT DISCRIMINATE ON ITS OWN, and it never could.** This fixture's
 * `region_3_3` has `longestShortestPath = 1`, so its per-tile charge IS its
 * coarse charge — measured 23 = 23 under the current block. The claim that
 * separates the two arms is the COUNT (≥ 3 such drops; the coarse arm produces
 * ONE manaChanged event for the whole run, V2b) together with the chain walk
 * below. The cap only keeps a single large coarse bite from being miscounted as
 * three small ones. The typed `30` this replaced was, for the old block's
 * 50/55/60, an accident of the same shape.
 */
const PER_TILE_CAP = (() => {
    const doc = JSON.parse(readFileSync(
        path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..', RULES), 'utf8'));
    ...
    console.log('  per-tile charges derived from the block: '
        + perTile.map((p) => `${p.region} ${p.moveCost}/${p.lsp}=${p.cost.toFixed(2)}`).join(', '));
    return Math.max(...perTile.map((p) => p.cost));
})();
...
async function main() {
     ...
     * events; no per-tile drops vs a run of them; all-zero XP vs XP on every
     * chain region; `substrateActionCompleted` `[]` vs a
     * non-empty list carrying the interrupted walk's `false` (measured
     * `[true,true,true,true,false]` under the current block; it was
     * `[true,true,true,false,true]` under the old one — the reset falls at a
     * different action because the costs moved, and the claim is the `false`,
     * not its index); `loopResetCount` 0 vs **1**; frozen in
     * `region_2_2` at index 1 vs the whole chain walked to `region_3_3`.
     *
     * ⚠ The per-tile FIGURES that used to stand here (16.67 · 13.75×3; XP
     * 16.67 / 41.25 / 60) were read off the OLD generator's block and are gone
     * with it. Under the one cost model this fixture's per-tile charges are
     * moveCost ÷ longestShortestPath = 50/3, 32/4, 23/1 — see the cap's
     * derivation below, which computes them rather than quoting them.
     *
    ...
    if (after.currentMana !== after.maxMana) {
        ...
        // the SAME derived figure, because that is exactly what the tolerance
        // is for (it was a second typed 30).
        const gap = after.maxMana - after.currentMana;
        if (gap > PER_TILE_CAP) {
            fail(`reset did not refill mana: ${after.currentMana}/${after.maxMana} `
                + `(gap ${gap.toFixed(2)} > one per-tile bite ${PER_TILE_CAP.toFixed(2)})`);
        }
    }
