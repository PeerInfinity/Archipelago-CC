 ...
 * SUBSTRATE'S OWN PAYLOAD DECLARATION, EVERY SLOT PASSES THE HUB'S SIDECAR
 * VALIDITY REPORT, AND EVERY AUTHORED PAYLOAD'S RULES EQUAL ITS DOCUMENT'S**
 * (PRESET SIDECARS slice D0, grown by V0 and G2b-1 into three layers:
 * `sidecarPayloadErrors` per entry, `sidecarIssues` per slot, then
 * `regionRuleAgreement` per region — see "THE SECOND LAYER" and "THE THIRD
 * LAYER" below; plan §7.4, ⚖ Q1 C: *"field descriptors with an optional
 * per-field JSON-Schema fragment, plus a corpus gate"*).
 *
