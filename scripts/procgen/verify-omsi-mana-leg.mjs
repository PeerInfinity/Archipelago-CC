...
takeBoxLockOrExit({ name: 'verify-omsi-mana-leg.mjs', kind: 'browser' });
...
async function main() {
    ...
    await page.evaluate(async () => {
        ...
        eventBus.subscribe('loopState:queuePausedUntilReset', (d) =>
            mark('queuePausedUntilReset', d), 'verify-omsi-mana');
        eventBus.subscribe('loopState:manualEntered', (d) =>
            mark('manualEntered', d), 'verify-omsi-mana');
        eventBus.subscribe('loopState:manualResumed', (d) =>
            mark('manualResumed', d), 'verify-omsi-mana');
    });
