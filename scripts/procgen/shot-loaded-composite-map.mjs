...
/**
 * ⛓ EVERYTHING THIS FILE DRIVES LIVES IN `main()`, AND `main()` RUNS ONLY
 * WHEN THIS FILE IS THE ENTRY POINT — the IMPORT door of
 * `check-procgen-help.mjs`. Before V2 the box lock, the browser launch and the
 * whole shot ran at module scope, so a bare `await import()` — which is what a
 * vitest file importing anything from this directory does — TOOK THE REAL BOX
 * and drove a headless Chromium. The pattern is `check-slice-records.mjs`'s.
 */
async function main() {
