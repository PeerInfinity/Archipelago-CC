 ...
 * twice. Every one of those was declared in the baseline's `wrote:` lists (dropped in G1) — the
...
const treeLine = WORKTREE
    ...
    : `## tree: IN PLACE (${IS_ENTRY_POINT ? '--in-place' : 'IMPORTED, not launched'}) `
      ...
      + 'measured heads that is two tracked files dirtied and five untracked droppings '
      + '(`--only=<file> --json` names what a door writes).';
if (!JSON_OUT) {
...
if (WRITE_BASELINE) {
    const head = HEAD;
    /**
     * ⛓ EACH ENTRY CARRIES WHAT THE GATE READS BACK — its name and the output
    ...
    console.log(`wrote ${BASELINE_FILE.split('/').pop()} — ${effectfulCount} of `
