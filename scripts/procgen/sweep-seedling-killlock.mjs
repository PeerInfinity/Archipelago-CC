 ...
 * instrument's SUBJECT went with them: its subject is `wall-gap-spinner-killlock`, retired into the room-aware `killgate` ELEMENT (arc-3 §13.2). The element's own census and certification table are §12's.
 *
 * ⛓ ITS LAST MEASUREMENT LIVES IN arc-3 kickoff §12 (the element's census + yield table) and §13.6(b)/(c) (the `SPINNER_OFFSET` sweep and the `span` domain this script's family carried).
 *
...
process.stderr.write(
    'sweep-seedling-killlock: RETIRED in PROCGEN ELEMENTS arc 3 slice 4c — its subject '
