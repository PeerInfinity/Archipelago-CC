"""
Access rules for Starcraft 2 WorldGen WorldGen.
...
# Helper functions
def _starcraft2worldgen_is_item_placement(state: "CollectionState", player: int) -> bool:
    return state.has_group('Missions', player, 0)
...
def _starcraft2worldgen_weapon_armor_upgrade_count(state: "CollectionState", player: int, upgrade_item) -> bool:
    count = 0
    if (self.generic_upgrade_missions > 0):
        if (not (_starcraft2worldgen_is_item_placement(state, player))) or ((self.logic_level == 3)):
            return 5
        else:
            count += _starcraft2worldgen_floor(state, player, ((100 / self.generic_upgrade_missions) * (state.count_group('Missions', player) / self.total_mission_count)))
