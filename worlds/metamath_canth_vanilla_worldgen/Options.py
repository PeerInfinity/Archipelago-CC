"""
Game options for Metamath Canth Vanilla WorldGen.

Auto-generated by world_generator.
"""

from dataclasses import dataclass
from Options import Choice, Range, Toggle, PerGameCommonOptions


class RandomizeItems(Toggle):
    """Enable item randomization.

    When disabled, items will be placed in their original locations.
    """
    display_name = "Randomize Items"
    default = True


class UseCanonicalOptions(Toggle):
    """Use canonical options for seed 1.

    When enabled and generating seed 1, options will be loaded from the
    _worldgen_options.json file to reproduce the exact original seed.
    This ensures deterministic output matching the original world export.
    """
    display_name = "Use Canonical Options"
    default = True

class AutoDownloadDatabase(Toggle):
    """Option for Auto-Download Database."""
    display_name = "Auto-Download Database"
    default = True

class RandomizeStartingStatements(Toggle):
    """Option for Randomize Starting Statements."""
    display_name = "Randomize Starting Statements"
    default = True

class StartingStatements(Range):
    """Option for Starting Statements %."""
    display_name = "Starting Statements %"
    range_start = 0
    range_end = 50
    default = 0

class Theorem(Choice):
    """Option for Theorem to Prove."""
    display_name = "Theorem to Prove"
    option_1p1e2 = 0
    option_2p2e4 = 1
    option_3p3e6 = 2
    option_4p4e8 = 3
    option_5p5e10 = 4
    option_2m1e1 = 5
    option_pm2_21 = 6
    option_pm2_43 = 7
    option_pm5_32 = 8
    option_con3i = 9
    option_syl = 10
    option_uneq12i = 11
    option_ineq12i = 12
    option_pwfi = 13
    option_canth = 14
    option_grplid = 15
    option_grpinveu = 16
    option_cos0 = 17
    option_sin0 = 18
    option_euclemma = 19
    option_wilth = 20
    option_dfac5 = 21
    default = 1

class VanillaPlacement(Toggle):
    """Option for Vanilla Item Placement."""
    display_name = "Vanilla Item Placement"
    default = False


@dataclass
class MetamathCanthVanillaWorldGenOptions(PerGameCommonOptions):
    """Options for Metamath Canth Vanilla WorldGen."""
    randomize_items: RandomizeItems
    use_canonical_options: UseCanonicalOptions
    auto_download_database: AutoDownloadDatabase
    randomize_starting_statements: RandomizeStartingStatements
    starting_statements: StartingStatements
    theorem: Theorem
    vanilla_placement: VanillaPlacement
